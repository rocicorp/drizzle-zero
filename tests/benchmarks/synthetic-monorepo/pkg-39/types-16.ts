...
type DeepMerge_3916<T, U> = {
  [K in keyof T | keyof U]: K extends keyof T & keyof U
    ? T[K] extends object ? U[K] extends object ? DeepMerge_3916<T[K], U[K]> : U[K] : U[K]
...
type Path_3916<T, D extends unknown[] = []> = D['length'] extends 6 ? never
  : T extends object ? { [K in keyof T & string]: K | `${K}.${Path_3916<T[K], [...D, unknown]>}` }[keyof T & string] : never;
...
type Val_3916<T> = {
  [K in keyof T]: T[K] extends string ? { t: 's'; min: number; max: number }
    ...
    : T[K] extends unknown[] ? { t: 'a'; items: Val_3916<T[K][number]> }
    : T[K] extends object ? { t: 'o'; props: Val_3916<T[K]> }
    : { t: 'u' };
};
...
type CK_3916 = `p39.t16.${'on' | 'off' | 'auto'}.${'dev' | 'stg' | 'prd'}.${'v1' | 'v2' | 'v3'}`;

export type { Entity_39_16, Registry_39_16, CK_3916, EP_3916, EV_3916, DeepMerge_3916 };
