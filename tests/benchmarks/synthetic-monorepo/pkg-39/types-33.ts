...
type DeepMerge_3933<T, U> = {
  [K in keyof T | keyof U]: K extends keyof T & keyof U
    ? T[K] extends object ? U[K] extends object ? DeepMerge_3933<T[K], U[K]> : U[K] : U[K]
...
type Path_3933<T, D extends unknown[] = []> = D['length'] extends 6 ? never
  : T extends object ? { [K in keyof T & string]: K | `${K}.${Path_3933<T[K], [...D, unknown]>}` }[keyof T & string] : never;
...
type Val_3933<T> = {
  [K in keyof T]: T[K] extends string ? { t: 's'; min: number; max: number }
    ...
    : T[K] extends unknown[] ? { t: 'a'; items: Val_3933<T[K][number]> }
    : T[K] extends object ? { t: 'o'; props: Val_3933<T[K]> }
    : { t: 'u' };
};
...
type CK_3933 = `p39.t33.${'on' | 'off' | 'auto'}.${'dev' | 'stg' | 'prd'}.${'v1' | 'v2' | 'v3'}`;

export type { Entity_39_33, Registry_39_33, CK_3933, EP_3933, EV_3933, DeepMerge_3933 };
