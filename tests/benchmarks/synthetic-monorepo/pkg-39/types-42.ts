...
type DeepMerge_3942<T, U> = {
  [K in keyof T | keyof U]: K extends keyof T & keyof U
    ? T[K] extends object ? U[K] extends object ? DeepMerge_3942<T[K], U[K]> : U[K] : U[K]
...
type Path_3942<T, D extends unknown[] = []> = D['length'] extends 6 ? never
  : T extends object ? { [K in keyof T & string]: K | `${K}.${Path_3942<T[K], [...D, unknown]>}` }[keyof T & string] : never;
...
type Val_3942<T> = {
  [K in keyof T]: T[K] extends string ? { t: 's'; min: number; max: number }
    ...
    : T[K] extends unknown[] ? { t: 'a'; items: Val_3942<T[K][number]> }
    : T[K] extends object ? { t: 'o'; props: Val_3942<T[K]> }
    : { t: 'u' };
};
...
type CK_3942 = `p39.t42.${'on' | 'off' | 'auto'}.${'dev' | 'stg' | 'prd'}.${'v1' | 'v2' | 'v3'}`;

export type { Entity_39_42, Registry_39_42, CK_3942, EP_3942, EV_3942, DeepMerge_3942 };
