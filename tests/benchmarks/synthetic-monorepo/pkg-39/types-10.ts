...
type DeepMerge_3910<T, U> = {
  [K in keyof T | keyof U]: K extends keyof T & keyof U
    ? T[K] extends object ? U[K] extends object ? DeepMerge_3910<T[K], U[K]> : U[K] : U[K]
...
type Path_3910<T, D extends unknown[] = []> = D['length'] extends 6 ? never
  : T extends object ? { [K in keyof T & string]: K | `${K}.${Path_3910<T[K], [...D, unknown]>}` }[keyof T & string] : never;
...
type Val_3910<T> = {
  [K in keyof T]: T[K] extends string ? { t: 's'; min: number; max: number }
    ...
    : T[K] extends unknown[] ? { t: 'a'; items: Val_3910<T[K][number]> }
    : T[K] extends object ? { t: 'o'; props: Val_3910<T[K]> }
    : { t: 'u' };
};
...
type CK_3910 = `p39.t10.${'on' | 'off' | 'auto'}.${'dev' | 'stg' | 'prd'}.${'v1' | 'v2' | 'v3'}`;

export type { Entity_39_10, Registry_39_10, CK_3910, EP_3910, EV_3910, DeepMerge_3910 };
