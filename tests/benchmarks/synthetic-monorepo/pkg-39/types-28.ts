...
type DeepMerge_3928<T, U> = {
  [K in keyof T | keyof U]: K extends keyof T & keyof U
    ? T[K] extends object ? U[K] extends object ? DeepMerge_3928<T[K], U[K]> : U[K] : U[K]
...
type Path_3928<T, D extends unknown[] = []> = D['length'] extends 6 ? never
  : T extends object ? { [K in keyof T & string]: K | `${K}.${Path_3928<T[K], [...D, unknown]>}` }[keyof T & string] : never;
...
type Val_3928<T> = {
  [K in keyof T]: T[K] extends string ? { t: 's'; min: number; max: number }
    ...
    : T[K] extends unknown[] ? { t: 'a'; items: Val_3928<T[K][number]> }
    : T[K] extends object ? { t: 'o'; props: Val_3928<T[K]> }
    : { t: 'u' };
};
...
type CK_3928 = `p39.t28.${'on' | 'off' | 'auto'}.${'dev' | 'stg' | 'prd'}.${'v1' | 'v2' | 'v3'}`;

export type { Entity_39_28, Registry_39_28, CK_3928, EP_3928, EV_3928, DeepMerge_3928 };
