...
type DeepMerge_3901<T, U> = {
  [K in keyof T | keyof U]: K extends keyof T & keyof U
    ? T[K] extends object ? U[K] extends object ? DeepMerge_3901<T[K], U[K]> : U[K] : U[K]
...
type Path_3901<T, D extends unknown[] = []> = D['length'] extends 6 ? never
  : T extends object ? { [K in keyof T & string]: K | `${K}.${Path_3901<T[K], [...D, unknown]>}` }[keyof T & string] : never;
...
type Val_3901<T> = {
  [K in keyof T]: T[K] extends string ? { t: 's'; min: number; max: number }
    ...
    : T[K] extends unknown[] ? { t: 'a'; items: Val_3901<T[K][number]> }
    : T[K] extends object ? { t: 'o'; props: Val_3901<T[K]> }
    : { t: 'u' };
};
...
type CK_3901 = `p39.t01.${'on' | 'off' | 'auto'}.${'dev' | 'stg' | 'prd'}.${'v1' | 'v2' | 'v3'}`;

export type { Entity_39_01, Registry_39_01, CK_3901, EP_3901, EV_3901, DeepMerge_3901 };
