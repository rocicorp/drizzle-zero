...
type DeepMerge_3913<T, U> = {
  [K in keyof T | keyof U]: K extends keyof T & keyof U
    ? T[K] extends object ? U[K] extends object ? DeepMerge_3913<T[K], U[K]> : U[K] : U[K]
...
type Path_3913<T, D extends unknown[] = []> = D['length'] extends 6 ? never
  : T extends object ? { [K in keyof T & string]: K | `${K}.${Path_3913<T[K], [...D, unknown]>}` }[keyof T & string] : never;
...
type Val_3913<T> = {
  [K in keyof T]: T[K] extends string ? { t: 's'; min: number; max: number }
    ...
    : T[K] extends unknown[] ? { t: 'a'; items: Val_3913<T[K][number]> }
    : T[K] extends object ? { t: 'o'; props: Val_3913<T[K]> }
    : { t: 'u' };
};
...
type CK_3913 = `p39.t13.${'on' | 'off' | 'auto'}.${'dev' | 'stg' | 'prd'}.${'v1' | 'v2' | 'v3'}`;

export type { Entity_39_13, Registry_39_13, CK_3913, EP_3913, EV_3913, DeepMerge_3913 };
