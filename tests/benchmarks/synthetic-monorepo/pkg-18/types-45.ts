...
type DeepMerge_1845<T, U> = {
  [K in keyof T | keyof U]: K extends keyof T & keyof U
    ? T[K] extends object ? U[K] extends object ? DeepMerge_1845<T[K], U[K]> : U[K] : U[K]
...
type Path_1845<T, D extends unknown[] = []> = D['length'] extends 6 ? never
  : T extends object ? { [K in keyof T & string]: K | `${K}.${Path_1845<T[K], [...D, unknown]>}` }[keyof T & string] : never;
...
type Val_1845<T> = {
  [K in keyof T]: T[K] extends string ? { t: 's'; min: number; max: number }
    ...
    : T[K] extends unknown[] ? { t: 'a'; items: Val_1845<T[K][number]> }
    : T[K] extends object ? { t: 'o'; props: Val_1845<T[K]> }
    : { t: 'u' };
};
...
type CK_1845 = `p18.t45.${'on' | 'off' | 'auto'}.${'dev' | 'stg' | 'prd'}.${'v1' | 'v2' | 'v3'}`;

export type { Entity_18_45, Registry_18_45, CK_1845, EP_1845, EV_1845, DeepMerge_1845 };
