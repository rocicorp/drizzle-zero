...
type DeepMerge_1843<T, U> = {
  [K in keyof T | keyof U]: K extends keyof T & keyof U
    ? T[K] extends object ? U[K] extends object ? DeepMerge_1843<T[K], U[K]> : U[K] : U[K]
...
type Path_1843<T, D extends unknown[] = []> = D['length'] extends 6 ? never
  : T extends object ? { [K in keyof T & string]: K | `${K}.${Path_1843<T[K], [...D, unknown]>}` }[keyof T & string] : never;
...
type Val_1843<T> = {
  [K in keyof T]: T[K] extends string ? { t: 's'; min: number; max: number }
    ...
    : T[K] extends unknown[] ? { t: 'a'; items: Val_1843<T[K][number]> }
    : T[K] extends object ? { t: 'o'; props: Val_1843<T[K]> }
    : { t: 'u' };
};
...
type CK_1843 = `p18.t43.${'on' | 'off' | 'auto'}.${'dev' | 'stg' | 'prd'}.${'v1' | 'v2' | 'v3'}`;

export type { Entity_18_43, Registry_18_43, CK_1843, EP_1843, EV_1843, DeepMerge_1843 };
