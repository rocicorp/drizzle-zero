...
type DeepMerge_1839<T, U> = {
  [K in keyof T | keyof U]: K extends keyof T & keyof U
    ? T[K] extends object ? U[K] extends object ? DeepMerge_1839<T[K], U[K]> : U[K] : U[K]
...
type Path_1839<T, D extends unknown[] = []> = D['length'] extends 6 ? never
  : T extends object ? { [K in keyof T & string]: K | `${K}.${Path_1839<T[K], [...D, unknown]>}` }[keyof T & string] : never;
...
type Val_1839<T> = {
  [K in keyof T]: T[K] extends string ? { t: 's'; min: number; max: number }
    ...
    : T[K] extends unknown[] ? { t: 'a'; items: Val_1839<T[K][number]> }
    : T[K] extends object ? { t: 'o'; props: Val_1839<T[K]> }
    : { t: 'u' };
};
...
type CK_1839 = `p18.t39.${'on' | 'off' | 'auto'}.${'dev' | 'stg' | 'prd'}.${'v1' | 'v2' | 'v3'}`;

export type { Entity_18_39, Registry_18_39, CK_1839, EP_1839, EV_1839, DeepMerge_1839 };
