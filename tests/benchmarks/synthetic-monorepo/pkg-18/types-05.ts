...
type DeepMerge_1805<T, U> = {
  [K in keyof T | keyof U]: K extends keyof T & keyof U
    ? T[K] extends object ? U[K] extends object ? DeepMerge_1805<T[K], U[K]> : U[K] : U[K]
...
type Path_1805<T, D extends unknown[] = []> = D['length'] extends 6 ? never
  : T extends object ? { [K in keyof T & string]: K | `${K}.${Path_1805<T[K], [...D, unknown]>}` }[keyof T & string] : never;
...
type Val_1805<T> = {
  [K in keyof T]: T[K] extends string ? { t: 's'; min: number; max: number }
    ...
    : T[K] extends unknown[] ? { t: 'a'; items: Val_1805<T[K][number]> }
    : T[K] extends object ? { t: 'o'; props: Val_1805<T[K]> }
    : { t: 'u' };
};
...
type CK_1805 = `p18.t05.${'on' | 'off' | 'auto'}.${'dev' | 'stg' | 'prd'}.${'v1' | 'v2' | 'v3'}`;

export type { Entity_18_05, Registry_18_05, CK_1805, EP_1805, EV_1805, DeepMerge_1805 };
