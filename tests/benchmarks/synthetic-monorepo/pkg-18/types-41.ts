...
type DeepMerge_1841<T, U> = {
  [K in keyof T | keyof U]: K extends keyof T & keyof U
    ? T[K] extends object ? U[K] extends object ? DeepMerge_1841<T[K], U[K]> : U[K] : U[K]
...
type Path_1841<T, D extends unknown[] = []> = D['length'] extends 6 ? never
  : T extends object ? { [K in keyof T & string]: K | `${K}.${Path_1841<T[K], [...D, unknown]>}` }[keyof T & string] : never;
...
type Val_1841<T> = {
  [K in keyof T]: T[K] extends string ? { t: 's'; min: number; max: number }
    ...
    : T[K] extends unknown[] ? { t: 'a'; items: Val_1841<T[K][number]> }
    : T[K] extends object ? { t: 'o'; props: Val_1841<T[K]> }
    : { t: 'u' };
};
...
type CK_1841 = `p18.t41.${'on' | 'off' | 'auto'}.${'dev' | 'stg' | 'prd'}.${'v1' | 'v2' | 'v3'}`;

export type { Entity_18_41, Registry_18_41, CK_1841, EP_1841, EV_1841, DeepMerge_1841 };
