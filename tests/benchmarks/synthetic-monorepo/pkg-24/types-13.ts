...
type DeepMerge_2413<T, U> = {
  [K in keyof T | keyof U]: K extends keyof T & keyof U
    ? T[K] extends object ? U[K] extends object ? DeepMerge_2413<T[K], U[K]> : U[K] : U[K]
...
type Path_2413<T, D extends unknown[] = []> = D['length'] extends 6 ? never
  : T extends object ? { [K in keyof T & string]: K | `${K}.${Path_2413<T[K], [...D, unknown]>}` }[keyof T & string] : never;
...
type Val_2413<T> = {
  [K in keyof T]: T[K] extends string ? { t: 's'; min: number; max: number }
    ...
    : T[K] extends unknown[] ? { t: 'a'; items: Val_2413<T[K][number]> }
    : T[K] extends object ? { t: 'o'; props: Val_2413<T[K]> }
    : { t: 'u' };
};
...
type CK_2413 = `p24.t13.${'on' | 'off' | 'auto'}.${'dev' | 'stg' | 'prd'}.${'v1' | 'v2' | 'v3'}`;

export type { Entity_24_13, Registry_24_13, CK_2413, EP_2413, EV_2413, DeepMerge_2413 };
