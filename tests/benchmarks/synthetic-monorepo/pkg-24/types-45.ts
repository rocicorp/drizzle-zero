...
type DeepMerge_2445<T, U> = {
  [K in keyof T | keyof U]: K extends keyof T & keyof U
    ? T[K] extends object ? U[K] extends object ? DeepMerge_2445<T[K], U[K]> : U[K] : U[K]
...
type Path_2445<T, D extends unknown[] = []> = D['length'] extends 6 ? never
  : T extends object ? { [K in keyof T & string]: K | `${K}.${Path_2445<T[K], [...D, unknown]>}` }[keyof T & string] : never;
...
type Val_2445<T> = {
  [K in keyof T]: T[K] extends string ? { t: 's'; min: number; max: number }
    ...
    : T[K] extends unknown[] ? { t: 'a'; items: Val_2445<T[K][number]> }
    : T[K] extends object ? { t: 'o'; props: Val_2445<T[K]> }
    : { t: 'u' };
};
...
type CK_2445 = `p24.t45.${'on' | 'off' | 'auto'}.${'dev' | 'stg' | 'prd'}.${'v1' | 'v2' | 'v3'}`;

export type { Entity_24_45, Registry_24_45, CK_2445, EP_2445, EV_2445, DeepMerge_2445 };
