...
type DeepMerge_2434<T, U> = {
  [K in keyof T | keyof U]: K extends keyof T & keyof U
    ? T[K] extends object ? U[K] extends object ? DeepMerge_2434<T[K], U[K]> : U[K] : U[K]
...
type Path_2434<T, D extends unknown[] = []> = D['length'] extends 6 ? never
  : T extends object ? { [K in keyof T & string]: K | `${K}.${Path_2434<T[K], [...D, unknown]>}` }[keyof T & string] : never;
...
type Val_2434<T> = {
  [K in keyof T]: T[K] extends string ? { t: 's'; min: number; max: number }
    ...
    : T[K] extends unknown[] ? { t: 'a'; items: Val_2434<T[K][number]> }
    : T[K] extends object ? { t: 'o'; props: Val_2434<T[K]> }
    : { t: 'u' };
};
...
type CK_2434 = `p24.t34.${'on' | 'off' | 'auto'}.${'dev' | 'stg' | 'prd'}.${'v1' | 'v2' | 'v3'}`;

export type { Entity_24_34, Registry_24_34, CK_2434, EP_2434, EV_2434, DeepMerge_2434 };
