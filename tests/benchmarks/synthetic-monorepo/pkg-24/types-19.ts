...
type DeepMerge_2419<T, U> = {
  [K in keyof T | keyof U]: K extends keyof T & keyof U
    ? T[K] extends object ? U[K] extends object ? DeepMerge_2419<T[K], U[K]> : U[K] : U[K]
...
type Path_2419<T, D extends unknown[] = []> = D['length'] extends 6 ? never
  : T extends object ? { [K in keyof T & string]: K | `${K}.${Path_2419<T[K], [...D, unknown]>}` }[keyof T & string] : never;
...
type Val_2419<T> = {
  [K in keyof T]: T[K] extends string ? { t: 's'; min: number; max: number }
    ...
    : T[K] extends unknown[] ? { t: 'a'; items: Val_2419<T[K][number]> }
    : T[K] extends object ? { t: 'o'; props: Val_2419<T[K]> }
    : { t: 'u' };
};
...
type CK_2419 = `p24.t19.${'on' | 'off' | 'auto'}.${'dev' | 'stg' | 'prd'}.${'v1' | 'v2' | 'v3'}`;

export type { Entity_24_19, Registry_24_19, CK_2419, EP_2419, EV_2419, DeepMerge_2419 };
