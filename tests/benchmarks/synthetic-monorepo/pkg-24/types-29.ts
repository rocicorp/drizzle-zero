...
type DeepMerge_2429<T, U> = {
  [K in keyof T | keyof U]: K extends keyof T & keyof U
    ? T[K] extends object ? U[K] extends object ? DeepMerge_2429<T[K], U[K]> : U[K] : U[K]
...
type Path_2429<T, D extends unknown[] = []> = D['length'] extends 6 ? never
  : T extends object ? { [K in keyof T & string]: K | `${K}.${Path_2429<T[K], [...D, unknown]>}` }[keyof T & string] : never;
...
type Val_2429<T> = {
  [K in keyof T]: T[K] extends string ? { t: 's'; min: number; max: number }
    ...
    : T[K] extends unknown[] ? { t: 'a'; items: Val_2429<T[K][number]> }
    : T[K] extends object ? { t: 'o'; props: Val_2429<T[K]> }
    : { t: 'u' };
};
...
type CK_2429 = `p24.t29.${'on' | 'off' | 'auto'}.${'dev' | 'stg' | 'prd'}.${'v1' | 'v2' | 'v3'}`;

export type { Entity_24_29, Registry_24_29, CK_2429, EP_2429, EV_2429, DeepMerge_2429 };
