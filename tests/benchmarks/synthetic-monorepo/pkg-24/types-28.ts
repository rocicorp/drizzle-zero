...
type DeepMerge_2428<T, U> = {
  [K in keyof T | keyof U]: K extends keyof T & keyof U
    ? T[K] extends object ? U[K] extends object ? DeepMerge_2428<T[K], U[K]> : U[K] : U[K]
...
type Path_2428<T, D extends unknown[] = []> = D['length'] extends 6 ? never
  : T extends object ? { [K in keyof T & string]: K | `${K}.${Path_2428<T[K], [...D, unknown]>}` }[keyof T & string] : never;
...
type Val_2428<T> = {
  [K in keyof T]: T[K] extends string ? { t: 's'; min: number; max: number }
    ...
    : T[K] extends unknown[] ? { t: 'a'; items: Val_2428<T[K][number]> }
    : T[K] extends object ? { t: 'o'; props: Val_2428<T[K]> }
    : { t: 'u' };
};
...
type CK_2428 = `p24.t28.${'on' | 'off' | 'auto'}.${'dev' | 'stg' | 'prd'}.${'v1' | 'v2' | 'v3'}`;

export type { Entity_24_28, Registry_24_28, CK_2428, EP_2428, EV_2428, DeepMerge_2428 };
