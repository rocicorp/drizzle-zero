...
type DeepMerge_2406<T, U> = {
  [K in keyof T | keyof U]: K extends keyof T & keyof U
    ? T[K] extends object ? U[K] extends object ? DeepMerge_2406<T[K], U[K]> : U[K] : U[K]
...
type Path_2406<T, D extends unknown[] = []> = D['length'] extends 6 ? never
  : T extends object ? { [K in keyof T & string]: K | `${K}.${Path_2406<T[K], [...D, unknown]>}` }[keyof T & string] : never;
...
type Val_2406<T> = {
  [K in keyof T]: T[K] extends string ? { t: 's'; min: number; max: number }
    ...
    : T[K] extends unknown[] ? { t: 'a'; items: Val_2406<T[K][number]> }
    : T[K] extends object ? { t: 'o'; props: Val_2406<T[K]> }
    : { t: 'u' };
};
...
type CK_2406 = `p24.t06.${'on' | 'off' | 'auto'}.${'dev' | 'stg' | 'prd'}.${'v1' | 'v2' | 'v3'}`;

export type { Entity_24_06, Registry_24_06, CK_2406, EP_2406, EV_2406, DeepMerge_2406 };
