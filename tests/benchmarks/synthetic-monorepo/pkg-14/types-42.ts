...
type DeepMerge_1442<T, U> = {
  [K in keyof T | keyof U]: K extends keyof T & keyof U
    ? T[K] extends object ? U[K] extends object ? DeepMerge_1442<T[K], U[K]> : U[K] : U[K]
...
type Path_1442<T, D extends unknown[] = []> = D['length'] extends 6 ? never
  : T extends object ? { [K in keyof T & string]: K | `${K}.${Path_1442<T[K], [...D, unknown]>}` }[keyof T & string] : never;
...
type Val_1442<T> = {
  [K in keyof T]: T[K] extends string ? { t: 's'; min: number; max: number }
    ...
    : T[K] extends unknown[] ? { t: 'a'; items: Val_1442<T[K][number]> }
    : T[K] extends object ? { t: 'o'; props: Val_1442<T[K]> }
    : { t: 'u' };
};
...
type CK_1442 = `p14.t42.${'on' | 'off' | 'auto'}.${'dev' | 'stg' | 'prd'}.${'v1' | 'v2' | 'v3'}`;

export type { Entity_14_42, Registry_14_42, CK_1442, EP_1442, EV_1442, DeepMerge_1442 };
