...
type DeepMerge_1444<T, U> = {
  [K in keyof T | keyof U]: K extends keyof T & keyof U
    ? T[K] extends object ? U[K] extends object ? DeepMerge_1444<T[K], U[K]> : U[K] : U[K]
...
type Path_1444<T, D extends unknown[] = []> = D['length'] extends 6 ? never
  : T extends object ? { [K in keyof T & string]: K | `${K}.${Path_1444<T[K], [...D, unknown]>}` }[keyof T & string] : never;
...
type Val_1444<T> = {
  [K in keyof T]: T[K] extends string ? { t: 's'; min: number; max: number }
    ...
    : T[K] extends unknown[] ? { t: 'a'; items: Val_1444<T[K][number]> }
    : T[K] extends object ? { t: 'o'; props: Val_1444<T[K]> }
    : { t: 'u' };
};
...
type CK_1444 = `p14.t44.${'on' | 'off' | 'auto'}.${'dev' | 'stg' | 'prd'}.${'v1' | 'v2' | 'v3'}`;

export type { Entity_14_44, Registry_14_44, CK_1444, EP_1444, EV_1444, DeepMerge_1444 };
