...
type DeepMerge_1437<T, U> = {
  [K in keyof T | keyof U]: K extends keyof T & keyof U
    ? T[K] extends object ? U[K] extends object ? DeepMerge_1437<T[K], U[K]> : U[K] : U[K]
...
type Path_1437<T, D extends unknown[] = []> = D['length'] extends 6 ? never
  : T extends object ? { [K in keyof T & string]: K | `${K}.${Path_1437<T[K], [...D, unknown]>}` }[keyof T & string] : never;
...
type Val_1437<T> = {
  [K in keyof T]: T[K] extends string ? { t: 's'; min: number; max: number }
    ...
    : T[K] extends unknown[] ? { t: 'a'; items: Val_1437<T[K][number]> }
    : T[K] extends object ? { t: 'o'; props: Val_1437<T[K]> }
    : { t: 'u' };
};
...
type CK_1437 = `p14.t37.${'on' | 'off' | 'auto'}.${'dev' | 'stg' | 'prd'}.${'v1' | 'v2' | 'v3'}`;

export type { Entity_14_37, Registry_14_37, CK_1437, EP_1437, EV_1437, DeepMerge_1437 };
