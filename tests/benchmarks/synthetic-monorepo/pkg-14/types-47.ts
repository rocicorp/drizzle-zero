...
type DeepMerge_1447<T, U> = {
  [K in keyof T | keyof U]: K extends keyof T & keyof U
    ? T[K] extends object ? U[K] extends object ? DeepMerge_1447<T[K], U[K]> : U[K] : U[K]
...
type Path_1447<T, D extends unknown[] = []> = D['length'] extends 6 ? never
  : T extends object ? { [K in keyof T & string]: K | `${K}.${Path_1447<T[K], [...D, unknown]>}` }[keyof T & string] : never;
...
type Val_1447<T> = {
  [K in keyof T]: T[K] extends string ? { t: 's'; min: number; max: number }
    ...
    : T[K] extends unknown[] ? { t: 'a'; items: Val_1447<T[K][number]> }
    : T[K] extends object ? { t: 'o'; props: Val_1447<T[K]> }
    : { t: 'u' };
};
...
type CK_1447 = `p14.t47.${'on' | 'off' | 'auto'}.${'dev' | 'stg' | 'prd'}.${'v1' | 'v2' | 'v3'}`;

export type { Entity_14_47, Registry_14_47, CK_1447, EP_1447, EV_1447, DeepMerge_1447 };
