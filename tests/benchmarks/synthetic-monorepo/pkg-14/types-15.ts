...
type DeepMerge_1415<T, U> = {
  [K in keyof T | keyof U]: K extends keyof T & keyof U
    ? T[K] extends object ? U[K] extends object ? DeepMerge_1415<T[K], U[K]> : U[K] : U[K]
...
type Path_1415<T, D extends unknown[] = []> = D['length'] extends 6 ? never
  : T extends object ? { [K in keyof T & string]: K | `${K}.${Path_1415<T[K], [...D, unknown]>}` }[keyof T & string] : never;
...
type Val_1415<T> = {
  [K in keyof T]: T[K] extends string ? { t: 's'; min: number; max: number }
    ...
    : T[K] extends unknown[] ? { t: 'a'; items: Val_1415<T[K][number]> }
    : T[K] extends object ? { t: 'o'; props: Val_1415<T[K]> }
    : { t: 'u' };
};
...
type CK_1415 = `p14.t15.${'on' | 'off' | 'auto'}.${'dev' | 'stg' | 'prd'}.${'v1' | 'v2' | 'v3'}`;

export type { Entity_14_15, Registry_14_15, CK_1415, EP_1415, EV_1415, DeepMerge_1415 };
