...
type DeepMerge_1433<T, U> = {
  [K in keyof T | keyof U]: K extends keyof T & keyof U
    ? T[K] extends object ? U[K] extends object ? DeepMerge_1433<T[K], U[K]> : U[K] : U[K]
...
type Path_1433<T, D extends unknown[] = []> = D['length'] extends 6 ? never
  : T extends object ? { [K in keyof T & string]: K | `${K}.${Path_1433<T[K], [...D, unknown]>}` }[keyof T & string] : never;
...
type Val_1433<T> = {
  [K in keyof T]: T[K] extends string ? { t: 's'; min: number; max: number }
    ...
    : T[K] extends unknown[] ? { t: 'a'; items: Val_1433<T[K][number]> }
    : T[K] extends object ? { t: 'o'; props: Val_1433<T[K]> }
    : { t: 'u' };
};
...
type CK_1433 = `p14.t33.${'on' | 'off' | 'auto'}.${'dev' | 'stg' | 'prd'}.${'v1' | 'v2' | 'v3'}`;

export type { Entity_14_33, Registry_14_33, CK_1433, EP_1433, EV_1433, DeepMerge_1433 };
