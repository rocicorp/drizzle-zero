...
type DeepMerge_1331<T, U> = {
  [K in keyof T | keyof U]: K extends keyof T & keyof U
    ? T[K] extends object ? U[K] extends object ? DeepMerge_1331<T[K], U[K]> : U[K] : U[K]
...
type Path_1331<T, D extends unknown[] = []> = D['length'] extends 6 ? never
  : T extends object ? { [K in keyof T & string]: K | `${K}.${Path_1331<T[K], [...D, unknown]>}` }[keyof T & string] : never;
...
type Val_1331<T> = {
  [K in keyof T]: T[K] extends string ? { t: 's'; min: number; max: number }
    ...
    : T[K] extends unknown[] ? { t: 'a'; items: Val_1331<T[K][number]> }
    : T[K] extends object ? { t: 'o'; props: Val_1331<T[K]> }
    : { t: 'u' };
};
...
type CK_1331 = `p13.t31.${'on' | 'off' | 'auto'}.${'dev' | 'stg' | 'prd'}.${'v1' | 'v2' | 'v3'}`;

export type { Entity_13_31, Registry_13_31, CK_1331, EP_1331, EV_1331, DeepMerge_1331 };
