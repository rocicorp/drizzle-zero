...
type DeepMerge_1340<T, U> = {
  [K in keyof T | keyof U]: K extends keyof T & keyof U
    ? T[K] extends object ? U[K] extends object ? DeepMerge_1340<T[K], U[K]> : U[K] : U[K]
...
type Path_1340<T, D extends unknown[] = []> = D['length'] extends 6 ? never
  : T extends object ? { [K in keyof T & string]: K | `${K}.${Path_1340<T[K], [...D, unknown]>}` }[keyof T & string] : never;
...
type Val_1340<T> = {
  [K in keyof T]: T[K] extends string ? { t: 's'; min: number; max: number }
    ...
    : T[K] extends unknown[] ? { t: 'a'; items: Val_1340<T[K][number]> }
    : T[K] extends object ? { t: 'o'; props: Val_1340<T[K]> }
    : { t: 'u' };
};
...
type CK_1340 = `p13.t40.${'on' | 'off' | 'auto'}.${'dev' | 'stg' | 'prd'}.${'v1' | 'v2' | 'v3'}`;

export type { Entity_13_40, Registry_13_40, CK_1340, EP_1340, EV_1340, DeepMerge_1340 };
