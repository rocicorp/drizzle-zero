...
type DeepMerge_1321<T, U> = {
  [K in keyof T | keyof U]: K extends keyof T & keyof U
    ? T[K] extends object ? U[K] extends object ? DeepMerge_1321<T[K], U[K]> : U[K] : U[K]
...
type Path_1321<T, D extends unknown[] = []> = D['length'] extends 6 ? never
  : T extends object ? { [K in keyof T & string]: K | `${K}.${Path_1321<T[K], [...D, unknown]>}` }[keyof T & string] : never;
...
type Val_1321<T> = {
  [K in keyof T]: T[K] extends string ? { t: 's'; min: number; max: number }
    ...
    : T[K] extends unknown[] ? { t: 'a'; items: Val_1321<T[K][number]> }
    : T[K] extends object ? { t: 'o'; props: Val_1321<T[K]> }
    : { t: 'u' };
};
...
type CK_1321 = `p13.t21.${'on' | 'off' | 'auto'}.${'dev' | 'stg' | 'prd'}.${'v1' | 'v2' | 'v3'}`;

export type { Entity_13_21, Registry_13_21, CK_1321, EP_1321, EV_1321, DeepMerge_1321 };
