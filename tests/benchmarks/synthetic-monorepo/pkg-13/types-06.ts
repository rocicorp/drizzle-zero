...
type DeepMerge_1306<T, U> = {
  [K in keyof T | keyof U]: K extends keyof T & keyof U
    ? T[K] extends object ? U[K] extends object ? DeepMerge_1306<T[K], U[K]> : U[K] : U[K]
...
type Path_1306<T, D extends unknown[] = []> = D['length'] extends 6 ? never
  : T extends object ? { [K in keyof T & string]: K | `${K}.${Path_1306<T[K], [...D, unknown]>}` }[keyof T & string] : never;
...
type Val_1306<T> = {
  [K in keyof T]: T[K] extends string ? { t: 's'; min: number; max: number }
    ...
    : T[K] extends unknown[] ? { t: 'a'; items: Val_1306<T[K][number]> }
    : T[K] extends object ? { t: 'o'; props: Val_1306<T[K]> }
    : { t: 'u' };
};
...
type CK_1306 = `p13.t06.${'on' | 'off' | 'auto'}.${'dev' | 'stg' | 'prd'}.${'v1' | 'v2' | 'v3'}`;

export type { Entity_13_06, Registry_13_06, CK_1306, EP_1306, EV_1306, DeepMerge_1306 };
