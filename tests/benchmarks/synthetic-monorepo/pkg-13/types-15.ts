...
type DeepMerge_1315<T, U> = {
  [K in keyof T | keyof U]: K extends keyof T & keyof U
    ? T[K] extends object ? U[K] extends object ? DeepMerge_1315<T[K], U[K]> : U[K] : U[K]
...
type Path_1315<T, D extends unknown[] = []> = D['length'] extends 6 ? never
  : T extends object ? { [K in keyof T & string]: K | `${K}.${Path_1315<T[K], [...D, unknown]>}` }[keyof T & string] : never;
...
type Val_1315<T> = {
  [K in keyof T]: T[K] extends string ? { t: 's'; min: number; max: number }
    ...
    : T[K] extends unknown[] ? { t: 'a'; items: Val_1315<T[K][number]> }
    : T[K] extends object ? { t: 'o'; props: Val_1315<T[K]> }
    : { t: 'u' };
};
...
type CK_1315 = `p13.t15.${'on' | 'off' | 'auto'}.${'dev' | 'stg' | 'prd'}.${'v1' | 'v2' | 'v3'}`;

export type { Entity_13_15, Registry_13_15, CK_1315, EP_1315, EV_1315, DeepMerge_1315 };
