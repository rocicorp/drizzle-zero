...
type DeepMerge_1308<T, U> = {
  [K in keyof T | keyof U]: K extends keyof T & keyof U
    ? T[K] extends object ? U[K] extends object ? DeepMerge_1308<T[K], U[K]> : U[K] : U[K]
...
type Path_1308<T, D extends unknown[] = []> = D['length'] extends 6 ? never
  : T extends object ? { [K in keyof T & string]: K | `${K}.${Path_1308<T[K], [...D, unknown]>}` }[keyof T & string] : never;
...
type Val_1308<T> = {
  [K in keyof T]: T[K] extends string ? { t: 's'; min: number; max: number }
    ...
    : T[K] extends unknown[] ? { t: 'a'; items: Val_1308<T[K][number]> }
    : T[K] extends object ? { t: 'o'; props: Val_1308<T[K]> }
    : { t: 'u' };
};
...
type CK_1308 = `p13.t08.${'on' | 'off' | 'auto'}.${'dev' | 'stg' | 'prd'}.${'v1' | 'v2' | 'v3'}`;

export type { Entity_13_08, Registry_13_08, CK_1308, EP_1308, EV_1308, DeepMerge_1308 };
