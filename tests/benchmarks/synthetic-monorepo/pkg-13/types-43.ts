...
type DeepMerge_1343<T, U> = {
  [K in keyof T | keyof U]: K extends keyof T & keyof U
    ? T[K] extends object ? U[K] extends object ? DeepMerge_1343<T[K], U[K]> : U[K] : U[K]
...
type Path_1343<T, D extends unknown[] = []> = D['length'] extends 6 ? never
  : T extends object ? { [K in keyof T & string]: K | `${K}.${Path_1343<T[K], [...D, unknown]>}` }[keyof T & string] : never;
...
type Val_1343<T> = {
  [K in keyof T]: T[K] extends string ? { t: 's'; min: number; max: number }
    ...
    : T[K] extends unknown[] ? { t: 'a'; items: Val_1343<T[K][number]> }
    : T[K] extends object ? { t: 'o'; props: Val_1343<T[K]> }
    : { t: 'u' };
};
...
type CK_1343 = `p13.t43.${'on' | 'off' | 'auto'}.${'dev' | 'stg' | 'prd'}.${'v1' | 'v2' | 'v3'}`;

export type { Entity_13_43, Registry_13_43, CK_1343, EP_1343, EV_1343, DeepMerge_1343 };
