...
type DeepMerge_0740<T, U> = {
  [K in keyof T | keyof U]: K extends keyof T & keyof U
    ? T[K] extends object ? U[K] extends object ? DeepMerge_0740<T[K], U[K]> : U[K] : U[K]
...
type Path_0740<T, D extends unknown[] = []> = D['length'] extends 6 ? never
  : T extends object ? { [K in keyof T & string]: K | `${K}.${Path_0740<T[K], [...D, unknown]>}` }[keyof T & string] : never;
...
type Val_0740<T> = {
  [K in keyof T]: T[K] extends string ? { t: 's'; min: number; max: number }
    ...
    : T[K] extends unknown[] ? { t: 'a'; items: Val_0740<T[K][number]> }
    : T[K] extends object ? { t: 'o'; props: Val_0740<T[K]> }
    : { t: 'u' };
};
...
type CK_0740 = `p07.t40.${'on' | 'off' | 'auto'}.${'dev' | 'stg' | 'prd'}.${'v1' | 'v2' | 'v3'}`;

export type { Entity_07_40, Registry_07_40, CK_0740, EP_0740, EV_0740, DeepMerge_0740 };
