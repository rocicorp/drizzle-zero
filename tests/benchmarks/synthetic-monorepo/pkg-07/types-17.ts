...
type DeepMerge_0717<T, U> = {
  [K in keyof T | keyof U]: K extends keyof T & keyof U
    ? T[K] extends object ? U[K] extends object ? DeepMerge_0717<T[K], U[K]> : U[K] : U[K]
...
type Path_0717<T, D extends unknown[] = []> = D['length'] extends 6 ? never
  : T extends object ? { [K in keyof T & string]: K | `${K}.${Path_0717<T[K], [...D, unknown]>}` }[keyof T & string] : never;
...
type Val_0717<T> = {
  [K in keyof T]: T[K] extends string ? { t: 's'; min: number; max: number }
    ...
    : T[K] extends unknown[] ? { t: 'a'; items: Val_0717<T[K][number]> }
    : T[K] extends object ? { t: 'o'; props: Val_0717<T[K]> }
    : { t: 'u' };
};
...
type CK_0717 = `p07.t17.${'on' | 'off' | 'auto'}.${'dev' | 'stg' | 'prd'}.${'v1' | 'v2' | 'v3'}`;

export type { Entity_07_17, Registry_07_17, CK_0717, EP_0717, EV_0717, DeepMerge_0717 };
