...
type DeepMerge_0725<T, U> = {
  [K in keyof T | keyof U]: K extends keyof T & keyof U
    ? T[K] extends object ? U[K] extends object ? DeepMerge_0725<T[K], U[K]> : U[K] : U[K]
...
type Path_0725<T, D extends unknown[] = []> = D['length'] extends 6 ? never
  : T extends object ? { [K in keyof T & string]: K | `${K}.${Path_0725<T[K], [...D, unknown]>}` }[keyof T & string] : never;
...
type Val_0725<T> = {
  [K in keyof T]: T[K] extends string ? { t: 's'; min: number; max: number }
    ...
    : T[K] extends unknown[] ? { t: 'a'; items: Val_0725<T[K][number]> }
    : T[K] extends object ? { t: 'o'; props: Val_0725<T[K]> }
    : { t: 'u' };
};
...
type CK_0725 = `p07.t25.${'on' | 'off' | 'auto'}.${'dev' | 'stg' | 'prd'}.${'v1' | 'v2' | 'v3'}`;

export type { Entity_07_25, Registry_07_25, CK_0725, EP_0725, EV_0725, DeepMerge_0725 };
