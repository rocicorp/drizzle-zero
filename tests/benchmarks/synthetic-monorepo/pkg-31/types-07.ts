...
type DeepMerge_3107<T, U> = {
  [K in keyof T | keyof U]: K extends keyof T & keyof U
    ? T[K] extends object ? U[K] extends object ? DeepMerge_3107<T[K], U[K]> : U[K] : U[K]
...
type Path_3107<T, D extends unknown[] = []> = D['length'] extends 6 ? never
  : T extends object ? { [K in keyof T & string]: K | `${K}.${Path_3107<T[K], [...D, unknown]>}` }[keyof T & string] : never;
...
type Val_3107<T> = {
  [K in keyof T]: T[K] extends string ? { t: 's'; min: number; max: number }
    ...
    : T[K] extends unknown[] ? { t: 'a'; items: Val_3107<T[K][number]> }
    : T[K] extends object ? { t: 'o'; props: Val_3107<T[K]> }
    : { t: 'u' };
};
...
type CK_3107 = `p31.t07.${'on' | 'off' | 'auto'}.${'dev' | 'stg' | 'prd'}.${'v1' | 'v2' | 'v3'}`;

export type { Entity_31_07, Registry_31_07, CK_3107, EP_3107, EV_3107, DeepMerge_3107 };
