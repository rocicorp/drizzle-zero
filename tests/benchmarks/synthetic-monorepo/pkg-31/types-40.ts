...
type DeepMerge_3140<T, U> = {
  [K in keyof T | keyof U]: K extends keyof T & keyof U
    ? T[K] extends object ? U[K] extends object ? DeepMerge_3140<T[K], U[K]> : U[K] : U[K]
...
type Path_3140<T, D extends unknown[] = []> = D['length'] extends 6 ? never
  : T extends object ? { [K in keyof T & string]: K | `${K}.${Path_3140<T[K], [...D, unknown]>}` }[keyof T & string] : never;
...
type Val_3140<T> = {
  [K in keyof T]: T[K] extends string ? { t: 's'; min: number; max: number }
    ...
    : T[K] extends unknown[] ? { t: 'a'; items: Val_3140<T[K][number]> }
    : T[K] extends object ? { t: 'o'; props: Val_3140<T[K]> }
    : { t: 'u' };
};
...
type CK_3140 = `p31.t40.${'on' | 'off' | 'auto'}.${'dev' | 'stg' | 'prd'}.${'v1' | 'v2' | 'v3'}`;

export type { Entity_31_40, Registry_31_40, CK_3140, EP_3140, EV_3140, DeepMerge_3140 };
