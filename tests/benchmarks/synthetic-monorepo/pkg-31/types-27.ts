...
type DeepMerge_3127<T, U> = {
  [K in keyof T | keyof U]: K extends keyof T & keyof U
    ? T[K] extends object ? U[K] extends object ? DeepMerge_3127<T[K], U[K]> : U[K] : U[K]
...
type Path_3127<T, D extends unknown[] = []> = D['length'] extends 6 ? never
  : T extends object ? { [K in keyof T & string]: K | `${K}.${Path_3127<T[K], [...D, unknown]>}` }[keyof T & string] : never;
...
type Val_3127<T> = {
  [K in keyof T]: T[K] extends string ? { t: 's'; min: number; max: number }
    ...
    : T[K] extends unknown[] ? { t: 'a'; items: Val_3127<T[K][number]> }
    : T[K] extends object ? { t: 'o'; props: Val_3127<T[K]> }
    : { t: 'u' };
};
...
type CK_3127 = `p31.t27.${'on' | 'off' | 'auto'}.${'dev' | 'stg' | 'prd'}.${'v1' | 'v2' | 'v3'}`;

export type { Entity_31_27, Registry_31_27, CK_3127, EP_3127, EV_3127, DeepMerge_3127 };
