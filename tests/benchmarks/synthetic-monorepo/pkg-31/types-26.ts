...
type DeepMerge_3126<T, U> = {
  [K in keyof T | keyof U]: K extends keyof T & keyof U
    ? T[K] extends object ? U[K] extends object ? DeepMerge_3126<T[K], U[K]> : U[K] : U[K]
...
type Path_3126<T, D extends unknown[] = []> = D['length'] extends 6 ? never
  : T extends object ? { [K in keyof T & string]: K | `${K}.${Path_3126<T[K], [...D, unknown]>}` }[keyof T & string] : never;
...
type Val_3126<T> = {
  [K in keyof T]: T[K] extends string ? { t: 's'; min: number; max: number }
    ...
    : T[K] extends unknown[] ? { t: 'a'; items: Val_3126<T[K][number]> }
    : T[K] extends object ? { t: 'o'; props: Val_3126<T[K]> }
    : { t: 'u' };
};
...
type CK_3126 = `p31.t26.${'on' | 'off' | 'auto'}.${'dev' | 'stg' | 'prd'}.${'v1' | 'v2' | 'v3'}`;

export type { Entity_31_26, Registry_31_26, CK_3126, EP_3126, EV_3126, DeepMerge_3126 };
