...
type DeepMerge_3119<T, U> = {
  [K in keyof T | keyof U]: K extends keyof T & keyof U
    ? T[K] extends object ? U[K] extends object ? DeepMerge_3119<T[K], U[K]> : U[K] : U[K]
...
type Path_3119<T, D extends unknown[] = []> = D['length'] extends 6 ? never
  : T extends object ? { [K in keyof T & string]: K | `${K}.${Path_3119<T[K], [...D, unknown]>}` }[keyof T & string] : never;
...
type Val_3119<T> = {
  [K in keyof T]: T[K] extends string ? { t: 's'; min: number; max: number }
    ...
    : T[K] extends unknown[] ? { t: 'a'; items: Val_3119<T[K][number]> }
    : T[K] extends object ? { t: 'o'; props: Val_3119<T[K]> }
    : { t: 'u' };
};
...
type CK_3119 = `p31.t19.${'on' | 'off' | 'auto'}.${'dev' | 'stg' | 'prd'}.${'v1' | 'v2' | 'v3'}`;

export type { Entity_31_19, Registry_31_19, CK_3119, EP_3119, EV_3119, DeepMerge_3119 };
