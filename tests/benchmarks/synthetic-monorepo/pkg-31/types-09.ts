...
type DeepMerge_3109<T, U> = {
  [K in keyof T | keyof U]: K extends keyof T & keyof U
    ? T[K] extends object ? U[K] extends object ? DeepMerge_3109<T[K], U[K]> : U[K] : U[K]
...
type Path_3109<T, D extends unknown[] = []> = D['length'] extends 6 ? never
  : T extends object ? { [K in keyof T & string]: K | `${K}.${Path_3109<T[K], [...D, unknown]>}` }[keyof T & string] : never;
...
type Val_3109<T> = {
  [K in keyof T]: T[K] extends string ? { t: 's'; min: number; max: number }
    ...
    : T[K] extends unknown[] ? { t: 'a'; items: Val_3109<T[K][number]> }
    : T[K] extends object ? { t: 'o'; props: Val_3109<T[K]> }
    : { t: 'u' };
};
...
type CK_3109 = `p31.t09.${'on' | 'off' | 'auto'}.${'dev' | 'stg' | 'prd'}.${'v1' | 'v2' | 'v3'}`;

export type { Entity_31_09, Registry_31_09, CK_3109, EP_3109, EV_3109, DeepMerge_3109 };
