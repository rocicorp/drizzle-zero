...
type DeepMerge_3131<T, U> = {
  [K in keyof T | keyof U]: K extends keyof T & keyof U
    ? T[K] extends object ? U[K] extends object ? DeepMerge_3131<T[K], U[K]> : U[K] : U[K]
...
type Path_3131<T, D extends unknown[] = []> = D['length'] extends 6 ? never
  : T extends object ? { [K in keyof T & string]: K | `${K}.${Path_3131<T[K], [...D, unknown]>}` }[keyof T & string] : never;
...
type Val_3131<T> = {
  [K in keyof T]: T[K] extends string ? { t: 's'; min: number; max: number }
    ...
    : T[K] extends unknown[] ? { t: 'a'; items: Val_3131<T[K][number]> }
    : T[K] extends object ? { t: 'o'; props: Val_3131<T[K]> }
    : { t: 'u' };
};
...
type CK_3131 = `p31.t31.${'on' | 'off' | 'auto'}.${'dev' | 'stg' | 'prd'}.${'v1' | 'v2' | 'v3'}`;

export type { Entity_31_31, Registry_31_31, CK_3131, EP_3131, EV_3131, DeepMerge_3131 };
