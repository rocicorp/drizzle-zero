...
type DeepMerge_2030<T, U> = {
  [K in keyof T | keyof U]: K extends keyof T & keyof U
    ? T[K] extends object ? U[K] extends object ? DeepMerge_2030<T[K], U[K]> : U[K] : U[K]
...
type Path_2030<T, D extends unknown[] = []> = D['length'] extends 6 ? never
  : T extends object ? { [K in keyof T & string]: K | `${K}.${Path_2030<T[K], [...D, unknown]>}` }[keyof T & string] : never;
...
type Val_2030<T> = {
  [K in keyof T]: T[K] extends string ? { t: 's'; min: number; max: number }
    ...
    : T[K] extends unknown[] ? { t: 'a'; items: Val_2030<T[K][number]> }
    : T[K] extends object ? { t: 'o'; props: Val_2030<T[K]> }
    : { t: 'u' };
};
...
type CK_2030 = `p20.t30.${'on' | 'off' | 'auto'}.${'dev' | 'stg' | 'prd'}.${'v1' | 'v2' | 'v3'}`;

export type { Entity_20_30, Registry_20_30, CK_2030, EP_2030, EV_2030, DeepMerge_2030 };
