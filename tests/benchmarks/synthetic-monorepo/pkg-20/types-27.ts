...
type DeepMerge_2027<T, U> = {
  [K in keyof T | keyof U]: K extends keyof T & keyof U
    ? T[K] extends object ? U[K] extends object ? DeepMerge_2027<T[K], U[K]> : U[K] : U[K]
...
type Path_2027<T, D extends unknown[] = []> = D['length'] extends 6 ? never
  : T extends object ? { [K in keyof T & string]: K | `${K}.${Path_2027<T[K], [...D, unknown]>}` }[keyof T & string] : never;
...
type Val_2027<T> = {
  [K in keyof T]: T[K] extends string ? { t: 's'; min: number; max: number }
    ...
    : T[K] extends unknown[] ? { t: 'a'; items: Val_2027<T[K][number]> }
    : T[K] extends object ? { t: 'o'; props: Val_2027<T[K]> }
    : { t: 'u' };
};
...
type CK_2027 = `p20.t27.${'on' | 'off' | 'auto'}.${'dev' | 'stg' | 'prd'}.${'v1' | 'v2' | 'v3'}`;

export type { Entity_20_27, Registry_20_27, CK_2027, EP_2027, EV_2027, DeepMerge_2027 };
