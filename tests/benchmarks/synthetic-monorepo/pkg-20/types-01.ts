...
type DeepMerge_2001<T, U> = {
  [K in keyof T | keyof U]: K extends keyof T & keyof U
    ? T[K] extends object ? U[K] extends object ? DeepMerge_2001<T[K], U[K]> : U[K] : U[K]
...
type Path_2001<T, D extends unknown[] = []> = D['length'] extends 6 ? never
  : T extends object ? { [K in keyof T & string]: K | `${K}.${Path_2001<T[K], [...D, unknown]>}` }[keyof T & string] : never;
...
type Val_2001<T> = {
  [K in keyof T]: T[K] extends string ? { t: 's'; min: number; max: number }
    ...
    : T[K] extends unknown[] ? { t: 'a'; items: Val_2001<T[K][number]> }
    : T[K] extends object ? { t: 'o'; props: Val_2001<T[K]> }
    : { t: 'u' };
};
...
type CK_2001 = `p20.t01.${'on' | 'off' | 'auto'}.${'dev' | 'stg' | 'prd'}.${'v1' | 'v2' | 'v3'}`;

export type { Entity_20_01, Registry_20_01, CK_2001, EP_2001, EV_2001, DeepMerge_2001 };
