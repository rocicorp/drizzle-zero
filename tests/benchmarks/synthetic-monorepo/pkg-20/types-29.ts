...
type DeepMerge_2029<T, U> = {
  [K in keyof T | keyof U]: K extends keyof T & keyof U
    ? T[K] extends object ? U[K] extends object ? DeepMerge_2029<T[K], U[K]> : U[K] : U[K]
...
type Path_2029<T, D extends unknown[] = []> = D['length'] extends 6 ? never
  : T extends object ? { [K in keyof T & string]: K | `${K}.${Path_2029<T[K], [...D, unknown]>}` }[keyof T & string] : never;
...
type Val_2029<T> = {
  [K in keyof T]: T[K] extends string ? { t: 's'; min: number; max: number }
    ...
    : T[K] extends unknown[] ? { t: 'a'; items: Val_2029<T[K][number]> }
    : T[K] extends object ? { t: 'o'; props: Val_2029<T[K]> }
    : { t: 'u' };
};
...
type CK_2029 = `p20.t29.${'on' | 'off' | 'auto'}.${'dev' | 'stg' | 'prd'}.${'v1' | 'v2' | 'v3'}`;

export type { Entity_20_29, Registry_20_29, CK_2029, EP_2029, EV_2029, DeepMerge_2029 };
