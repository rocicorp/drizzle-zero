...
type DeepMerge_3406<T, U> = {
  [K in keyof T | keyof U]: K extends keyof T & keyof U
    ? T[K] extends object ? U[K] extends object ? DeepMerge_3406<T[K], U[K]> : U[K] : U[K]
...
type Path_3406<T, D extends unknown[] = []> = D['length'] extends 6 ? never
  : T extends object ? { [K in keyof T & string]: K | `${K}.${Path_3406<T[K], [...D, unknown]>}` }[keyof T & string] : never;
...
type Val_3406<T> = {
  [K in keyof T]: T[K] extends string ? { t: 's'; min: number; max: number }
    ...
    : T[K] extends unknown[] ? { t: 'a'; items: Val_3406<T[K][number]> }
    : T[K] extends object ? { t: 'o'; props: Val_3406<T[K]> }
    : { t: 'u' };
};
...
type CK_3406 = `p34.t06.${'on' | 'off' | 'auto'}.${'dev' | 'stg' | 'prd'}.${'v1' | 'v2' | 'v3'}`;

export type { Entity_34_06, Registry_34_06, CK_3406, EP_3406, EV_3406, DeepMerge_3406 };
