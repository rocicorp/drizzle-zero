...
type DeepMerge_3442<T, U> = {
  [K in keyof T | keyof U]: K extends keyof T & keyof U
    ? T[K] extends object ? U[K] extends object ? DeepMerge_3442<T[K], U[K]> : U[K] : U[K]
...
type Path_3442<T, D extends unknown[] = []> = D['length'] extends 6 ? never
  : T extends object ? { [K in keyof T & string]: K | `${K}.${Path_3442<T[K], [...D, unknown]>}` }[keyof T & string] : never;
...
type Val_3442<T> = {
  [K in keyof T]: T[K] extends string ? { t: 's'; min: number; max: number }
    ...
    : T[K] extends unknown[] ? { t: 'a'; items: Val_3442<T[K][number]> }
    : T[K] extends object ? { t: 'o'; props: Val_3442<T[K]> }
    : { t: 'u' };
};
...
type CK_3442 = `p34.t42.${'on' | 'off' | 'auto'}.${'dev' | 'stg' | 'prd'}.${'v1' | 'v2' | 'v3'}`;

export type { Entity_34_42, Registry_34_42, CK_3442, EP_3442, EV_3442, DeepMerge_3442 };
