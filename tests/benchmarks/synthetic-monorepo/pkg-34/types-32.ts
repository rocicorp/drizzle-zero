...
type DeepMerge_3432<T, U> = {
  [K in keyof T | keyof U]: K extends keyof T & keyof U
    ? T[K] extends object ? U[K] extends object ? DeepMerge_3432<T[K], U[K]> : U[K] : U[K]
...
type Path_3432<T, D extends unknown[] = []> = D['length'] extends 6 ? never
  : T extends object ? { [K in keyof T & string]: K | `${K}.${Path_3432<T[K], [...D, unknown]>}` }[keyof T & string] : never;
...
type Val_3432<T> = {
  [K in keyof T]: T[K] extends string ? { t: 's'; min: number; max: number }
    ...
    : T[K] extends unknown[] ? { t: 'a'; items: Val_3432<T[K][number]> }
    : T[K] extends object ? { t: 'o'; props: Val_3432<T[K]> }
    : { t: 'u' };
};
...
type CK_3432 = `p34.t32.${'on' | 'off' | 'auto'}.${'dev' | 'stg' | 'prd'}.${'v1' | 'v2' | 'v3'}`;

export type { Entity_34_32, Registry_34_32, CK_3432, EP_3432, EV_3432, DeepMerge_3432 };
