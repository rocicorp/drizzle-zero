...
type DeepMerge_0401<T, U> = {
  [K in keyof T | keyof U]: K extends keyof T & keyof U
    ? T[K] extends object ? U[K] extends object ? DeepMerge_0401<T[K], U[K]> : U[K] : U[K]
...
type Path_0401<T, D extends unknown[] = []> = D['length'] extends 6 ? never
  : T extends object ? { [K in keyof T & string]: K | `${K}.${Path_0401<T[K], [...D, unknown]>}` }[keyof T & string] : never;
...
type Val_0401<T> = {
  [K in keyof T]: T[K] extends string ? { t: 's'; min: number; max: number }
    ...
    : T[K] extends unknown[] ? { t: 'a'; items: Val_0401<T[K][number]> }
    : T[K] extends object ? { t: 'o'; props: Val_0401<T[K]> }
    : { t: 'u' };
};
...
type CK_0401 = `p04.t01.${'on' | 'off' | 'auto'}.${'dev' | 'stg' | 'prd'}.${'v1' | 'v2' | 'v3'}`;

export type { Entity_04_01, Registry_04_01, CK_0401, EP_0401, EV_0401, DeepMerge_0401 };
