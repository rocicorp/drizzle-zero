...
type DeepMerge_0431<T, U> = {
  [K in keyof T | keyof U]: K extends keyof T & keyof U
    ? T[K] extends object ? U[K] extends object ? DeepMerge_0431<T[K], U[K]> : U[K] : U[K]
...
type Path_0431<T, D extends unknown[] = []> = D['length'] extends 6 ? never
  : T extends object ? { [K in keyof T & string]: K | `${K}.${Path_0431<T[K], [...D, unknown]>}` }[keyof T & string] : never;
...
type Val_0431<T> = {
  [K in keyof T]: T[K] extends string ? { t: 's'; min: number; max: number }
    ...
    : T[K] extends unknown[] ? { t: 'a'; items: Val_0431<T[K][number]> }
    : T[K] extends object ? { t: 'o'; props: Val_0431<T[K]> }
    : { t: 'u' };
};
...
type CK_0431 = `p04.t31.${'on' | 'off' | 'auto'}.${'dev' | 'stg' | 'prd'}.${'v1' | 'v2' | 'v3'}`;

export type { Entity_04_31, Registry_04_31, CK_0431, EP_0431, EV_0431, DeepMerge_0431 };
