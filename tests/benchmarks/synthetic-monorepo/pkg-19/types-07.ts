...
type DeepMerge_1907<T, U> = {
  [K in keyof T | keyof U]: K extends keyof T & keyof U
    ? T[K] extends object ? U[K] extends object ? DeepMerge_1907<T[K], U[K]> : U[K] : U[K]
...
type Path_1907<T, D extends unknown[] = []> = D['length'] extends 6 ? never
  : T extends object ? { [K in keyof T & string]: K | `${K}.${Path_1907<T[K], [...D, unknown]>}` }[keyof T & string] : never;
...
type Val_1907<T> = {
  [K in keyof T]: T[K] extends string ? { t: 's'; min: number; max: number }
    ...
    : T[K] extends unknown[] ? { t: 'a'; items: Val_1907<T[K][number]> }
    : T[K] extends object ? { t: 'o'; props: Val_1907<T[K]> }
    : { t: 'u' };
};
...
type CK_1907 = `p19.t07.${'on' | 'off' | 'auto'}.${'dev' | 'stg' | 'prd'}.${'v1' | 'v2' | 'v3'}`;

export type { Entity_19_07, Registry_19_07, CK_1907, EP_1907, EV_1907, DeepMerge_1907 };
