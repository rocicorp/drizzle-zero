...
type DeepMerge_1905<T, U> = {
  [K in keyof T | keyof U]: K extends keyof T & keyof U
    ? T[K] extends object ? U[K] extends object ? DeepMerge_1905<T[K], U[K]> : U[K] : U[K]
...
type Path_1905<T, D extends unknown[] = []> = D['length'] extends 6 ? never
  : T extends object ? { [K in keyof T & string]: K | `${K}.${Path_1905<T[K], [...D, unknown]>}` }[keyof T & string] : never;
...
type Val_1905<T> = {
  [K in keyof T]: T[K] extends string ? { t: 's'; min: number; max: number }
    ...
    : T[K] extends unknown[] ? { t: 'a'; items: Val_1905<T[K][number]> }
    : T[K] extends object ? { t: 'o'; props: Val_1905<T[K]> }
    : { t: 'u' };
};
...
type CK_1905 = `p19.t05.${'on' | 'off' | 'auto'}.${'dev' | 'stg' | 'prd'}.${'v1' | 'v2' | 'v3'}`;

export type { Entity_19_05, Registry_19_05, CK_1905, EP_1905, EV_1905, DeepMerge_1905 };
