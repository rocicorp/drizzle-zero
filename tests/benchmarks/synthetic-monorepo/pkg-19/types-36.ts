...
type DeepMerge_1936<T, U> = {
  [K in keyof T | keyof U]: K extends keyof T & keyof U
    ? T[K] extends object ? U[K] extends object ? DeepMerge_1936<T[K], U[K]> : U[K] : U[K]
...
type Path_1936<T, D extends unknown[] = []> = D['length'] extends 6 ? never
  : T extends object ? { [K in keyof T & string]: K | `${K}.${Path_1936<T[K], [...D, unknown]>}` }[keyof T & string] : never;
...
type Val_1936<T> = {
  [K in keyof T]: T[K] extends string ? { t: 's'; min: number; max: number }
    ...
    : T[K] extends unknown[] ? { t: 'a'; items: Val_1936<T[K][number]> }
    : T[K] extends object ? { t: 'o'; props: Val_1936<T[K]> }
    : { t: 'u' };
};
...
type CK_1936 = `p19.t36.${'on' | 'off' | 'auto'}.${'dev' | 'stg' | 'prd'}.${'v1' | 'v2' | 'v3'}`;

export type { Entity_19_36, Registry_19_36, CK_1936, EP_1936, EV_1936, DeepMerge_1936 };
