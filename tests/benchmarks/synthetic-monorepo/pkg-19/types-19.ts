...
type DeepMerge_1919<T, U> = {
  [K in keyof T | keyof U]: K extends keyof T & keyof U
    ? T[K] extends object ? U[K] extends object ? DeepMerge_1919<T[K], U[K]> : U[K] : U[K]
...
type Path_1919<T, D extends unknown[] = []> = D['length'] extends 6 ? never
  : T extends object ? { [K in keyof T & string]: K | `${K}.${Path_1919<T[K], [...D, unknown]>}` }[keyof T & string] : never;
...
type Val_1919<T> = {
  [K in keyof T]: T[K] extends string ? { t: 's'; min: number; max: number }
    ...
    : T[K] extends unknown[] ? { t: 'a'; items: Val_1919<T[K][number]> }
    : T[K] extends object ? { t: 'o'; props: Val_1919<T[K]> }
    : { t: 'u' };
};
...
type CK_1919 = `p19.t19.${'on' | 'off' | 'auto'}.${'dev' | 'stg' | 'prd'}.${'v1' | 'v2' | 'v3'}`;

export type { Entity_19_19, Registry_19_19, CK_1919, EP_1919, EV_1919, DeepMerge_1919 };
