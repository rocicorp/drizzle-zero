...
type DeepMerge_1939<T, U> = {
  [K in keyof T | keyof U]: K extends keyof T & keyof U
    ? T[K] extends object ? U[K] extends object ? DeepMerge_1939<T[K], U[K]> : U[K] : U[K]
...
type Path_1939<T, D extends unknown[] = []> = D['length'] extends 6 ? never
  : T extends object ? { [K in keyof T & string]: K | `${K}.${Path_1939<T[K], [...D, unknown]>}` }[keyof T & string] : never;
...
type Val_1939<T> = {
  [K in keyof T]: T[K] extends string ? { t: 's'; min: number; max: number }
    ...
    : T[K] extends unknown[] ? { t: 'a'; items: Val_1939<T[K][number]> }
    : T[K] extends object ? { t: 'o'; props: Val_1939<T[K]> }
    : { t: 'u' };
};
...
type CK_1939 = `p19.t39.${'on' | 'off' | 'auto'}.${'dev' | 'stg' | 'prd'}.${'v1' | 'v2' | 'v3'}`;

export type { Entity_19_39, Registry_19_39, CK_1939, EP_1939, EV_1939, DeepMerge_1939 };
