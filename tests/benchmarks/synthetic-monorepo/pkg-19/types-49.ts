...
type DeepMerge_1949<T, U> = {
  [K in keyof T | keyof U]: K extends keyof T & keyof U
    ? T[K] extends object ? U[K] extends object ? DeepMerge_1949<T[K], U[K]> : U[K] : U[K]
...
type Path_1949<T, D extends unknown[] = []> = D['length'] extends 6 ? never
  : T extends object ? { [K in keyof T & string]: K | `${K}.${Path_1949<T[K], [...D, unknown]>}` }[keyof T & string] : never;
...
type Val_1949<T> = {
  [K in keyof T]: T[K] extends string ? { t: 's'; min: number; max: number }
    ...
    : T[K] extends unknown[] ? { t: 'a'; items: Val_1949<T[K][number]> }
    : T[K] extends object ? { t: 'o'; props: Val_1949<T[K]> }
    : { t: 'u' };
};
...
type CK_1949 = `p19.t49.${'on' | 'off' | 'auto'}.${'dev' | 'stg' | 'prd'}.${'v1' | 'v2' | 'v3'}`;

export type { Entity_19_49, Registry_19_49, CK_1949, EP_1949, EV_1949, DeepMerge_1949 };
