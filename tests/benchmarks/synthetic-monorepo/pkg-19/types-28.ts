...
type DeepMerge_1928<T, U> = {
  [K in keyof T | keyof U]: K extends keyof T & keyof U
    ? T[K] extends object ? U[K] extends object ? DeepMerge_1928<T[K], U[K]> : U[K] : U[K]
...
type Path_1928<T, D extends unknown[] = []> = D['length'] extends 6 ? never
  : T extends object ? { [K in keyof T & string]: K | `${K}.${Path_1928<T[K], [...D, unknown]>}` }[keyof T & string] : never;
...
type Val_1928<T> = {
  [K in keyof T]: T[K] extends string ? { t: 's'; min: number; max: number }
    ...
    : T[K] extends unknown[] ? { t: 'a'; items: Val_1928<T[K][number]> }
    : T[K] extends object ? { t: 'o'; props: Val_1928<T[K]> }
    : { t: 'u' };
};
...
type CK_1928 = `p19.t28.${'on' | 'off' | 'auto'}.${'dev' | 'stg' | 'prd'}.${'v1' | 'v2' | 'v3'}`;

export type { Entity_19_28, Registry_19_28, CK_1928, EP_1928, EV_1928, DeepMerge_1928 };
