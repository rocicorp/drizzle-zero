...
type DeepMerge_1223<T, U> = {
  [K in keyof T | keyof U]: K extends keyof T & keyof U
    ? T[K] extends object ? U[K] extends object ? DeepMerge_1223<T[K], U[K]> : U[K] : U[K]
...
type Path_1223<T, D extends unknown[] = []> = D['length'] extends 6 ? never
  : T extends object ? { [K in keyof T & string]: K | `${K}.${Path_1223<T[K], [...D, unknown]>}` }[keyof T & string] : never;
...
type Val_1223<T> = {
  [K in keyof T]: T[K] extends string ? { t: 's'; min: number; max: number }
    ...
    : T[K] extends unknown[] ? { t: 'a'; items: Val_1223<T[K][number]> }
    : T[K] extends object ? { t: 'o'; props: Val_1223<T[K]> }
    : { t: 'u' };
};
...
type CK_1223 = `p12.t23.${'on' | 'off' | 'auto'}.${'dev' | 'stg' | 'prd'}.${'v1' | 'v2' | 'v3'}`;

export type { Entity_12_23, Registry_12_23, CK_1223, EP_1223, EV_1223, DeepMerge_1223 };
