...
type DeepMerge_1213<T, U> = {
  [K in keyof T | keyof U]: K extends keyof T & keyof U
    ? T[K] extends object ? U[K] extends object ? DeepMerge_1213<T[K], U[K]> : U[K] : U[K]
...
type Path_1213<T, D extends unknown[] = []> = D['length'] extends 6 ? never
  : T extends object ? { [K in keyof T & string]: K | `${K}.${Path_1213<T[K], [...D, unknown]>}` }[keyof T & string] : never;
...
type Val_1213<T> = {
  [K in keyof T]: T[K] extends string ? { t: 's'; min: number; max: number }
    ...
    : T[K] extends unknown[] ? { t: 'a'; items: Val_1213<T[K][number]> }
    : T[K] extends object ? { t: 'o'; props: Val_1213<T[K]> }
    : { t: 'u' };
};
...
type CK_1213 = `p12.t13.${'on' | 'off' | 'auto'}.${'dev' | 'stg' | 'prd'}.${'v1' | 'v2' | 'v3'}`;

export type { Entity_12_13, Registry_12_13, CK_1213, EP_1213, EV_1213, DeepMerge_1213 };
