...
type DeepMerge_1248<T, U> = {
  [K in keyof T | keyof U]: K extends keyof T & keyof U
    ? T[K] extends object ? U[K] extends object ? DeepMerge_1248<T[K], U[K]> : U[K] : U[K]
...
type Path_1248<T, D extends unknown[] = []> = D['length'] extends 6 ? never
  : T extends object ? { [K in keyof T & string]: K | `${K}.${Path_1248<T[K], [...D, unknown]>}` }[keyof T & string] : never;
...
type Val_1248<T> = {
  [K in keyof T]: T[K] extends string ? { t: 's'; min: number; max: number }
    ...
    : T[K] extends unknown[] ? { t: 'a'; items: Val_1248<T[K][number]> }
    : T[K] extends object ? { t: 'o'; props: Val_1248<T[K]> }
    : { t: 'u' };
};
...
type CK_1248 = `p12.t48.${'on' | 'off' | 'auto'}.${'dev' | 'stg' | 'prd'}.${'v1' | 'v2' | 'v3'}`;

export type { Entity_12_48, Registry_12_48, CK_1248, EP_1248, EV_1248, DeepMerge_1248 };
