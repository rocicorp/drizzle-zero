...
type DeepMerge_1233<T, U> = {
  [K in keyof T | keyof U]: K extends keyof T & keyof U
    ? T[K] extends object ? U[K] extends object ? DeepMerge_1233<T[K], U[K]> : U[K] : U[K]
...
type Path_1233<T, D extends unknown[] = []> = D['length'] extends 6 ? never
  : T extends object ? { [K in keyof T & string]: K | `${K}.${Path_1233<T[K], [...D, unknown]>}` }[keyof T & string] : never;
...
type Val_1233<T> = {
  [K in keyof T]: T[K] extends string ? { t: 's'; min: number; max: number }
    ...
    : T[K] extends unknown[] ? { t: 'a'; items: Val_1233<T[K][number]> }
    : T[K] extends object ? { t: 'o'; props: Val_1233<T[K]> }
    : { t: 'u' };
};
...
type CK_1233 = `p12.t33.${'on' | 'off' | 'auto'}.${'dev' | 'stg' | 'prd'}.${'v1' | 'v2' | 'v3'}`;

export type { Entity_12_33, Registry_12_33, CK_1233, EP_1233, EV_1233, DeepMerge_1233 };
