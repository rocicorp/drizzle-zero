...
type DeepMerge_1249<T, U> = {
  [K in keyof T | keyof U]: K extends keyof T & keyof U
    ? T[K] extends object ? U[K] extends object ? DeepMerge_1249<T[K], U[K]> : U[K] : U[K]
...
type Path_1249<T, D extends unknown[] = []> = D['length'] extends 6 ? never
  : T extends object ? { [K in keyof T & string]: K | `${K}.${Path_1249<T[K], [...D, unknown]>}` }[keyof T & string] : never;
...
type Val_1249<T> = {
  [K in keyof T]: T[K] extends string ? { t: 's'; min: number; max: number }
    ...
    : T[K] extends unknown[] ? { t: 'a'; items: Val_1249<T[K][number]> }
    : T[K] extends object ? { t: 'o'; props: Val_1249<T[K]> }
    : { t: 'u' };
};
...
type CK_1249 = `p12.t49.${'on' | 'off' | 'auto'}.${'dev' | 'stg' | 'prd'}.${'v1' | 'v2' | 'v3'}`;

export type { Entity_12_49, Registry_12_49, CK_1249, EP_1249, EV_1249, DeepMerge_1249 };
