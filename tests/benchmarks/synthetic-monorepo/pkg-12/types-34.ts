...
type DeepMerge_1234<T, U> = {
  [K in keyof T | keyof U]: K extends keyof T & keyof U
    ? T[K] extends object ? U[K] extends object ? DeepMerge_1234<T[K], U[K]> : U[K] : U[K]
...
type Path_1234<T, D extends unknown[] = []> = D['length'] extends 6 ? never
  : T extends object ? { [K in keyof T & string]: K | `${K}.${Path_1234<T[K], [...D, unknown]>}` }[keyof T & string] : never;
...
type Val_1234<T> = {
  [K in keyof T]: T[K] extends string ? { t: 's'; min: number; max: number }
    ...
    : T[K] extends unknown[] ? { t: 'a'; items: Val_1234<T[K][number]> }
    : T[K] extends object ? { t: 'o'; props: Val_1234<T[K]> }
    : { t: 'u' };
};
...
type CK_1234 = `p12.t34.${'on' | 'off' | 'auto'}.${'dev' | 'stg' | 'prd'}.${'v1' | 'v2' | 'v3'}`;

export type { Entity_12_34, Registry_12_34, CK_1234, EP_1234, EV_1234, DeepMerge_1234 };
