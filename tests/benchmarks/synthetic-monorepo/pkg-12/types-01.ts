...
type DeepMerge_1201<T, U> = {
  [K in keyof T | keyof U]: K extends keyof T & keyof U
    ? T[K] extends object ? U[K] extends object ? DeepMerge_1201<T[K], U[K]> : U[K] : U[K]
...
type Path_1201<T, D extends unknown[] = []> = D['length'] extends 6 ? never
  : T extends object ? { [K in keyof T & string]: K | `${K}.${Path_1201<T[K], [...D, unknown]>}` }[keyof T & string] : never;
...
type Val_1201<T> = {
  [K in keyof T]: T[K] extends string ? { t: 's'; min: number; max: number }
    ...
    : T[K] extends unknown[] ? { t: 'a'; items: Val_1201<T[K][number]> }
    : T[K] extends object ? { t: 'o'; props: Val_1201<T[K]> }
    : { t: 'u' };
};
...
type CK_1201 = `p12.t01.${'on' | 'off' | 'auto'}.${'dev' | 'stg' | 'prd'}.${'v1' | 'v2' | 'v3'}`;

export type { Entity_12_01, Registry_12_01, CK_1201, EP_1201, EV_1201, DeepMerge_1201 };
