...
type DeepMerge_1232<T, U> = {
  [K in keyof T | keyof U]: K extends keyof T & keyof U
    ? T[K] extends object ? U[K] extends object ? DeepMerge_1232<T[K], U[K]> : U[K] : U[K]
...
type Path_1232<T, D extends unknown[] = []> = D['length'] extends 6 ? never
  : T extends object ? { [K in keyof T & string]: K | `${K}.${Path_1232<T[K], [...D, unknown]>}` }[keyof T & string] : never;
...
type Val_1232<T> = {
  [K in keyof T]: T[K] extends string ? { t: 's'; min: number; max: number }
    ...
    : T[K] extends unknown[] ? { t: 'a'; items: Val_1232<T[K][number]> }
    : T[K] extends object ? { t: 'o'; props: Val_1232<T[K]> }
    : { t: 'u' };
};
...
type CK_1232 = `p12.t32.${'on' | 'off' | 'auto'}.${'dev' | 'stg' | 'prd'}.${'v1' | 'v2' | 'v3'}`;

export type { Entity_12_32, Registry_12_32, CK_1232, EP_1232, EV_1232, DeepMerge_1232 };
