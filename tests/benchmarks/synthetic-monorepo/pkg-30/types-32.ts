...
type DeepMerge_3032<T, U> = {
  [K in keyof T | keyof U]: K extends keyof T & keyof U
    ? T[K] extends object ? U[K] extends object ? DeepMerge_3032<T[K], U[K]> : U[K] : U[K]
...
type Path_3032<T, D extends unknown[] = []> = D['length'] extends 6 ? never
  : T extends object ? { [K in keyof T & string]: K | `${K}.${Path_3032<T[K], [...D, unknown]>}` }[keyof T & string] : never;
...
type Val_3032<T> = {
  [K in keyof T]: T[K] extends string ? { t: 's'; min: number; max: number }
    ...
    : T[K] extends unknown[] ? { t: 'a'; items: Val_3032<T[K][number]> }
    : T[K] extends object ? { t: 'o'; props: Val_3032<T[K]> }
    : { t: 'u' };
};
...
type CK_3032 = `p30.t32.${'on' | 'off' | 'auto'}.${'dev' | 'stg' | 'prd'}.${'v1' | 'v2' | 'v3'}`;

export type { Entity_30_32, Registry_30_32, CK_3032, EP_3032, EV_3032, DeepMerge_3032 };
