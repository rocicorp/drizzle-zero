...
type DeepMerge_3022<T, U> = {
  [K in keyof T | keyof U]: K extends keyof T & keyof U
    ? T[K] extends object ? U[K] extends object ? DeepMerge_3022<T[K], U[K]> : U[K] : U[K]
...
type Path_3022<T, D extends unknown[] = []> = D['length'] extends 6 ? never
  : T extends object ? { [K in keyof T & string]: K | `${K}.${Path_3022<T[K], [...D, unknown]>}` }[keyof T & string] : never;
...
type Val_3022<T> = {
  [K in keyof T]: T[K] extends string ? { t: 's'; min: number; max: number }
    ...
    : T[K] extends unknown[] ? { t: 'a'; items: Val_3022<T[K][number]> }
    : T[K] extends object ? { t: 'o'; props: Val_3022<T[K]> }
    : { t: 'u' };
};
...
type CK_3022 = `p30.t22.${'on' | 'off' | 'auto'}.${'dev' | 'stg' | 'prd'}.${'v1' | 'v2' | 'v3'}`;

export type { Entity_30_22, Registry_30_22, CK_3022, EP_3022, EV_3022, DeepMerge_3022 };
