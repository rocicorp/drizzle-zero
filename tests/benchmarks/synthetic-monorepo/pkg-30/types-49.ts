...
type DeepMerge_3049<T, U> = {
  [K in keyof T | keyof U]: K extends keyof T & keyof U
    ? T[K] extends object ? U[K] extends object ? DeepMerge_3049<T[K], U[K]> : U[K] : U[K]
...
type Path_3049<T, D extends unknown[] = []> = D['length'] extends 6 ? never
  : T extends object ? { [K in keyof T & string]: K | `${K}.${Path_3049<T[K], [...D, unknown]>}` }[keyof T & string] : never;
...
type Val_3049<T> = {
  [K in keyof T]: T[K] extends string ? { t: 's'; min: number; max: number }
    ...
    : T[K] extends unknown[] ? { t: 'a'; items: Val_3049<T[K][number]> }
    : T[K] extends object ? { t: 'o'; props: Val_3049<T[K]> }
    : { t: 'u' };
};
...
type CK_3049 = `p30.t49.${'on' | 'off' | 'auto'}.${'dev' | 'stg' | 'prd'}.${'v1' | 'v2' | 'v3'}`;

export type { Entity_30_49, Registry_30_49, CK_3049, EP_3049, EV_3049, DeepMerge_3049 };
