...
type DeepMerge_3008<T, U> = {
  [K in keyof T | keyof U]: K extends keyof T & keyof U
    ? T[K] extends object ? U[K] extends object ? DeepMerge_3008<T[K], U[K]> : U[K] : U[K]
...
type Path_3008<T, D extends unknown[] = []> = D['length'] extends 6 ? never
  : T extends object ? { [K in keyof T & string]: K | `${K}.${Path_3008<T[K], [...D, unknown]>}` }[keyof T & string] : never;
...
type Val_3008<T> = {
  [K in keyof T]: T[K] extends string ? { t: 's'; min: number; max: number }
    ...
    : T[K] extends unknown[] ? { t: 'a'; items: Val_3008<T[K][number]> }
    : T[K] extends object ? { t: 'o'; props: Val_3008<T[K]> }
    : { t: 'u' };
};
...
type CK_3008 = `p30.t08.${'on' | 'off' | 'auto'}.${'dev' | 'stg' | 'prd'}.${'v1' | 'v2' | 'v3'}`;

export type { Entity_30_08, Registry_30_08, CK_3008, EP_3008, EV_3008, DeepMerge_3008 };
