...
type DeepMerge_3013<T, U> = {
  [K in keyof T | keyof U]: K extends keyof T & keyof U
    ? T[K] extends object ? U[K] extends object ? DeepMerge_3013<T[K], U[K]> : U[K] : U[K]
...
type Path_3013<T, D extends unknown[] = []> = D['length'] extends 6 ? never
  : T extends object ? { [K in keyof T & string]: K | `${K}.${Path_3013<T[K], [...D, unknown]>}` }[keyof T & string] : never;
...
type Val_3013<T> = {
  [K in keyof T]: T[K] extends string ? { t: 's'; min: number; max: number }
    ...
    : T[K] extends unknown[] ? { t: 'a'; items: Val_3013<T[K][number]> }
    : T[K] extends object ? { t: 'o'; props: Val_3013<T[K]> }
    : { t: 'u' };
};
...
type CK_3013 = `p30.t13.${'on' | 'off' | 'auto'}.${'dev' | 'stg' | 'prd'}.${'v1' | 'v2' | 'v3'}`;

export type { Entity_30_13, Registry_30_13, CK_3013, EP_3013, EV_3013, DeepMerge_3013 };
