...
type DeepMerge_0539<T, U> = {
  [K in keyof T | keyof U]: K extends keyof T & keyof U
    ? T[K] extends object ? U[K] extends object ? DeepMerge_0539<T[K], U[K]> : U[K] : U[K]
...
type Path_0539<T, D extends unknown[] = []> = D['length'] extends 6 ? never
  : T extends object ? { [K in keyof T & string]: K | `${K}.${Path_0539<T[K], [...D, unknown]>}` }[keyof T & string] : never;
...
type Val_0539<T> = {
  [K in keyof T]: T[K] extends string ? { t: 's'; min: number; max: number }
    ...
    : T[K] extends unknown[] ? { t: 'a'; items: Val_0539<T[K][number]> }
    : T[K] extends object ? { t: 'o'; props: Val_0539<T[K]> }
    : { t: 'u' };
};
...
type CK_0539 = `p05.t39.${'on' | 'off' | 'auto'}.${'dev' | 'stg' | 'prd'}.${'v1' | 'v2' | 'v3'}`;

export type { Entity_05_39, Registry_05_39, CK_0539, EP_0539, EV_0539, DeepMerge_0539 };
