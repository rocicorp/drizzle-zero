...
type DeepMerge_1549<T, U> = {
  [K in keyof T | keyof U]: K extends keyof T & keyof U
    ? T[K] extends object ? U[K] extends object ? DeepMerge_1549<T[K], U[K]> : U[K] : U[K]
...
type Path_1549<T, D extends unknown[] = []> = D['length'] extends 6 ? never
  : T extends object ? { [K in keyof T & string]: K | `${K}.${Path_1549<T[K], [...D, unknown]>}` }[keyof T & string] : never;
...
type Val_1549<T> = {
  [K in keyof T]: T[K] extends string ? { t: 's'; min: number; max: number }
    ...
    : T[K] extends unknown[] ? { t: 'a'; items: Val_1549<T[K][number]> }
    : T[K] extends object ? { t: 'o'; props: Val_1549<T[K]> }
    : { t: 'u' };
};
...
type CK_1549 = `p15.t49.${'on' | 'off' | 'auto'}.${'dev' | 'stg' | 'prd'}.${'v1' | 'v2' | 'v3'}`;

export type { Entity_15_49, Registry_15_49, CK_1549, EP_1549, EV_1549, DeepMerge_1549 };
