...
type DeepMerge_1517<T, U> = {
  [K in keyof T | keyof U]: K extends keyof T & keyof U
    ? T[K] extends object ? U[K] extends object ? DeepMerge_1517<T[K], U[K]> : U[K] : U[K]
...
type Path_1517<T, D extends unknown[] = []> = D['length'] extends 6 ? never
  : T extends object ? { [K in keyof T & string]: K | `${K}.${Path_1517<T[K], [...D, unknown]>}` }[keyof T & string] : never;
...
type Val_1517<T> = {
  [K in keyof T]: T[K] extends string ? { t: 's'; min: number; max: number }
    ...
    : T[K] extends unknown[] ? { t: 'a'; items: Val_1517<T[K][number]> }
    : T[K] extends object ? { t: 'o'; props: Val_1517<T[K]> }
    : { t: 'u' };
};
...
type CK_1517 = `p15.t17.${'on' | 'off' | 'auto'}.${'dev' | 'stg' | 'prd'}.${'v1' | 'v2' | 'v3'}`;

export type { Entity_15_17, Registry_15_17, CK_1517, EP_1517, EV_1517, DeepMerge_1517 };
