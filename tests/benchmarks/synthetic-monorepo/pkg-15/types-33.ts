...
type DeepMerge_1533<T, U> = {
  [K in keyof T | keyof U]: K extends keyof T & keyof U
    ? T[K] extends object ? U[K] extends object ? DeepMerge_1533<T[K], U[K]> : U[K] : U[K]
...
type Path_1533<T, D extends unknown[] = []> = D['length'] extends 6 ? never
  : T extends object ? { [K in keyof T & string]: K | `${K}.${Path_1533<T[K], [...D, unknown]>}` }[keyof T & string] : never;
...
type Val_1533<T> = {
  [K in keyof T]: T[K] extends string ? { t: 's'; min: number; max: number }
    ...
    : T[K] extends unknown[] ? { t: 'a'; items: Val_1533<T[K][number]> }
    : T[K] extends object ? { t: 'o'; props: Val_1533<T[K]> }
    : { t: 'u' };
};
...
type CK_1533 = `p15.t33.${'on' | 'off' | 'auto'}.${'dev' | 'stg' | 'prd'}.${'v1' | 'v2' | 'v3'}`;

export type { Entity_15_33, Registry_15_33, CK_1533, EP_1533, EV_1533, DeepMerge_1533 };
