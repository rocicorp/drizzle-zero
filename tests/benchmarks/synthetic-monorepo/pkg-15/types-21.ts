...
type DeepMerge_1521<T, U> = {
  [K in keyof T | keyof U]: K extends keyof T & keyof U
    ? T[K] extends object ? U[K] extends object ? DeepMerge_1521<T[K], U[K]> : U[K] : U[K]
...
type Path_1521<T, D extends unknown[] = []> = D['length'] extends 6 ? never
  : T extends object ? { [K in keyof T & string]: K | `${K}.${Path_1521<T[K], [...D, unknown]>}` }[keyof T & string] : never;
...
type Val_1521<T> = {
  [K in keyof T]: T[K] extends string ? { t: 's'; min: number; max: number }
    ...
    : T[K] extends unknown[] ? { t: 'a'; items: Val_1521<T[K][number]> }
    : T[K] extends object ? { t: 'o'; props: Val_1521<T[K]> }
    : { t: 'u' };
};
...
type CK_1521 = `p15.t21.${'on' | 'off' | 'auto'}.${'dev' | 'stg' | 'prd'}.${'v1' | 'v2' | 'v3'}`;

export type { Entity_15_21, Registry_15_21, CK_1521, EP_1521, EV_1521, DeepMerge_1521 };
