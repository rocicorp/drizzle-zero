...
type DeepMerge_1522<T, U> = {
  [K in keyof T | keyof U]: K extends keyof T & keyof U
    ? T[K] extends object ? U[K] extends object ? DeepMerge_1522<T[K], U[K]> : U[K] : U[K]
...
type Path_1522<T, D extends unknown[] = []> = D['length'] extends 6 ? never
  : T extends object ? { [K in keyof T & string]: K | `${K}.${Path_1522<T[K], [...D, unknown]>}` }[keyof T & string] : never;
...
type Val_1522<T> = {
  [K in keyof T]: T[K] extends string ? { t: 's'; min: number; max: number }
    ...
    : T[K] extends unknown[] ? { t: 'a'; items: Val_1522<T[K][number]> }
    : T[K] extends object ? { t: 'o'; props: Val_1522<T[K]> }
    : { t: 'u' };
};
...
type CK_1522 = `p15.t22.${'on' | 'off' | 'auto'}.${'dev' | 'stg' | 'prd'}.${'v1' | 'v2' | 'v3'}`;

export type { Entity_15_22, Registry_15_22, CK_1522, EP_1522, EV_1522, DeepMerge_1522 };
