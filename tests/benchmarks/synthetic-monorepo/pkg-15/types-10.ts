...
type DeepMerge_1510<T, U> = {
  [K in keyof T | keyof U]: K extends keyof T & keyof U
    ? T[K] extends object ? U[K] extends object ? DeepMerge_1510<T[K], U[K]> : U[K] : U[K]
...
type Path_1510<T, D extends unknown[] = []> = D['length'] extends 6 ? never
  : T extends object ? { [K in keyof T & string]: K | `${K}.${Path_1510<T[K], [...D, unknown]>}` }[keyof T & string] : never;
...
type Val_1510<T> = {
  [K in keyof T]: T[K] extends string ? { t: 's'; min: number; max: number }
    ...
    : T[K] extends unknown[] ? { t: 'a'; items: Val_1510<T[K][number]> }
    : T[K] extends object ? { t: 'o'; props: Val_1510<T[K]> }
    : { t: 'u' };
};
...
type CK_1510 = `p15.t10.${'on' | 'off' | 'auto'}.${'dev' | 'stg' | 'prd'}.${'v1' | 'v2' | 'v3'}`;

export type { Entity_15_10, Registry_15_10, CK_1510, EP_1510, EV_1510, DeepMerge_1510 };
