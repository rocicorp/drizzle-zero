...
type DeepMerge_1724<T, U> = {
  [K in keyof T | keyof U]: K extends keyof T & keyof U
    ? T[K] extends object ? U[K] extends object ? DeepMerge_1724<T[K], U[K]> : U[K] : U[K]
...
type Path_1724<T, D extends unknown[] = []> = D['length'] extends 6 ? never
  : T extends object ? { [K in keyof T & string]: K | `${K}.${Path_1724<T[K], [...D, unknown]>}` }[keyof T & string] : never;
...
type Val_1724<T> = {
  [K in keyof T]: T[K] extends string ? { t: 's'; min: number; max: number }
    ...
    : T[K] extends unknown[] ? { t: 'a'; items: Val_1724<T[K][number]> }
    : T[K] extends object ? { t: 'o'; props: Val_1724<T[K]> }
    : { t: 'u' };
};
...
type CK_1724 = `p17.t24.${'on' | 'off' | 'auto'}.${'dev' | 'stg' | 'prd'}.${'v1' | 'v2' | 'v3'}`;

export type { Entity_17_24, Registry_17_24, CK_1724, EP_1724, EV_1724, DeepMerge_1724 };
