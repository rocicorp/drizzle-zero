...
type DeepMerge_1719<T, U> = {
  [K in keyof T | keyof U]: K extends keyof T & keyof U
    ? T[K] extends object ? U[K] extends object ? DeepMerge_1719<T[K], U[K]> : U[K] : U[K]
...
type Path_1719<T, D extends unknown[] = []> = D['length'] extends 6 ? never
  : T extends object ? { [K in keyof T & string]: K | `${K}.${Path_1719<T[K], [...D, unknown]>}` }[keyof T & string] : never;
...
type Val_1719<T> = {
  [K in keyof T]: T[K] extends string ? { t: 's'; min: number; max: number }
    ...
    : T[K] extends unknown[] ? { t: 'a'; items: Val_1719<T[K][number]> }
    : T[K] extends object ? { t: 'o'; props: Val_1719<T[K]> }
    : { t: 'u' };
};
...
type CK_1719 = `p17.t19.${'on' | 'off' | 'auto'}.${'dev' | 'stg' | 'prd'}.${'v1' | 'v2' | 'v3'}`;

export type { Entity_17_19, Registry_17_19, CK_1719, EP_1719, EV_1719, DeepMerge_1719 };
