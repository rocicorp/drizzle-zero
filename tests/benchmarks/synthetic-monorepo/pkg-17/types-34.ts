...
type DeepMerge_1734<T, U> = {
  [K in keyof T | keyof U]: K extends keyof T & keyof U
    ? T[K] extends object ? U[K] extends object ? DeepMerge_1734<T[K], U[K]> : U[K] : U[K]
...
type Path_1734<T, D extends unknown[] = []> = D['length'] extends 6 ? never
  : T extends object ? { [K in keyof T & string]: K | `${K}.${Path_1734<T[K], [...D, unknown]>}` }[keyof T & string] : never;
...
type Val_1734<T> = {
  [K in keyof T]: T[K] extends string ? { t: 's'; min: number; max: number }
    ...
    : T[K] extends unknown[] ? { t: 'a'; items: Val_1734<T[K][number]> }
    : T[K] extends object ? { t: 'o'; props: Val_1734<T[K]> }
    : { t: 'u' };
};
...
type CK_1734 = `p17.t34.${'on' | 'off' | 'auto'}.${'dev' | 'stg' | 'prd'}.${'v1' | 'v2' | 'v3'}`;

export type { Entity_17_34, Registry_17_34, CK_1734, EP_1734, EV_1734, DeepMerge_1734 };
