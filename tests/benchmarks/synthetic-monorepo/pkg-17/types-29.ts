...
type DeepMerge_1729<T, U> = {
  [K in keyof T | keyof U]: K extends keyof T & keyof U
    ? T[K] extends object ? U[K] extends object ? DeepMerge_1729<T[K], U[K]> : U[K] : U[K]
...
type Path_1729<T, D extends unknown[] = []> = D['length'] extends 6 ? never
  : T extends object ? { [K in keyof T & string]: K | `${K}.${Path_1729<T[K], [...D, unknown]>}` }[keyof T & string] : never;
...
type Val_1729<T> = {
  [K in keyof T]: T[K] extends string ? { t: 's'; min: number; max: number }
    ...
    : T[K] extends unknown[] ? { t: 'a'; items: Val_1729<T[K][number]> }
    : T[K] extends object ? { t: 'o'; props: Val_1729<T[K]> }
    : { t: 'u' };
};
...
type CK_1729 = `p17.t29.${'on' | 'off' | 'auto'}.${'dev' | 'stg' | 'prd'}.${'v1' | 'v2' | 'v3'}`;

export type { Entity_17_29, Registry_17_29, CK_1729, EP_1729, EV_1729, DeepMerge_1729 };
