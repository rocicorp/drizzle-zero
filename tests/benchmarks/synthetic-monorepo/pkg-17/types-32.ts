...
type DeepMerge_1732<T, U> = {
  [K in keyof T | keyof U]: K extends keyof T & keyof U
    ? T[K] extends object ? U[K] extends object ? DeepMerge_1732<T[K], U[K]> : U[K] : U[K]
...
type Path_1732<T, D extends unknown[] = []> = D['length'] extends 6 ? never
  : T extends object ? { [K in keyof T & string]: K | `${K}.${Path_1732<T[K], [...D, unknown]>}` }[keyof T & string] : never;
...
type Val_1732<T> = {
  [K in keyof T]: T[K] extends string ? { t: 's'; min: number; max: number }
    ...
    : T[K] extends unknown[] ? { t: 'a'; items: Val_1732<T[K][number]> }
    : T[K] extends object ? { t: 'o'; props: Val_1732<T[K]> }
    : { t: 'u' };
};
...
type CK_1732 = `p17.t32.${'on' | 'off' | 'auto'}.${'dev' | 'stg' | 'prd'}.${'v1' | 'v2' | 'v3'}`;

export type { Entity_17_32, Registry_17_32, CK_1732, EP_1732, EV_1732, DeepMerge_1732 };
