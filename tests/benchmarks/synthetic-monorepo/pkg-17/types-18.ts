...
type DeepMerge_1718<T, U> = {
  [K in keyof T | keyof U]: K extends keyof T & keyof U
    ? T[K] extends object ? U[K] extends object ? DeepMerge_1718<T[K], U[K]> : U[K] : U[K]
...
type Path_1718<T, D extends unknown[] = []> = D['length'] extends 6 ? never
  : T extends object ? { [K in keyof T & string]: K | `${K}.${Path_1718<T[K], [...D, unknown]>}` }[keyof T & string] : never;
...
type Val_1718<T> = {
  [K in keyof T]: T[K] extends string ? { t: 's'; min: number; max: number }
    ...
    : T[K] extends unknown[] ? { t: 'a'; items: Val_1718<T[K][number]> }
    : T[K] extends object ? { t: 'o'; props: Val_1718<T[K]> }
    : { t: 'u' };
};
...
type CK_1718 = `p17.t18.${'on' | 'off' | 'auto'}.${'dev' | 'stg' | 'prd'}.${'v1' | 'v2' | 'v3'}`;

export type { Entity_17_18, Registry_17_18, CK_1718, EP_1718, EV_1718, DeepMerge_1718 };
