...
type DeepMerge_1730<T, U> = {
  [K in keyof T | keyof U]: K extends keyof T & keyof U
    ? T[K] extends object ? U[K] extends object ? DeepMerge_1730<T[K], U[K]> : U[K] : U[K]
...
type Path_1730<T, D extends unknown[] = []> = D['length'] extends 6 ? never
  : T extends object ? { [K in keyof T & string]: K | `${K}.${Path_1730<T[K], [...D, unknown]>}` }[keyof T & string] : never;
...
type Val_1730<T> = {
  [K in keyof T]: T[K] extends string ? { t: 's'; min: number; max: number }
    ...
    : T[K] extends unknown[] ? { t: 'a'; items: Val_1730<T[K][number]> }
    : T[K] extends object ? { t: 'o'; props: Val_1730<T[K]> }
    : { t: 'u' };
};
...
type CK_1730 = `p17.t30.${'on' | 'off' | 'auto'}.${'dev' | 'stg' | 'prd'}.${'v1' | 'v2' | 'v3'}`;

export type { Entity_17_30, Registry_17_30, CK_1730, EP_1730, EV_1730, DeepMerge_1730 };
