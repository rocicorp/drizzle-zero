...
type DeepMerge_0647<T, U> = {
  [K in keyof T | keyof U]: K extends keyof T & keyof U
    ? T[K] extends object ? U[K] extends object ? DeepMerge_0647<T[K], U[K]> : U[K] : U[K]
...
type Path_0647<T, D extends unknown[] = []> = D['length'] extends 6 ? never
  : T extends object ? { [K in keyof T & string]: K | `${K}.${Path_0647<T[K], [...D, unknown]>}` }[keyof T & string] : never;
...
type Val_0647<T> = {
  [K in keyof T]: T[K] extends string ? { t: 's'; min: number; max: number }
    ...
    : T[K] extends unknown[] ? { t: 'a'; items: Val_0647<T[K][number]> }
    : T[K] extends object ? { t: 'o'; props: Val_0647<T[K]> }
    : { t: 'u' };
};
...
type CK_0647 = `p06.t47.${'on' | 'off' | 'auto'}.${'dev' | 'stg' | 'prd'}.${'v1' | 'v2' | 'v3'}`;

export type { Entity_06_47, Registry_06_47, CK_0647, EP_0647, EV_0647, DeepMerge_0647 };
