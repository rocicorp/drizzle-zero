...
type DeepMerge_0616<T, U> = {
  [K in keyof T | keyof U]: K extends keyof T & keyof U
    ? T[K] extends object ? U[K] extends object ? DeepMerge_0616<T[K], U[K]> : U[K] : U[K]
...
type Path_0616<T, D extends unknown[] = []> = D['length'] extends 6 ? never
  : T extends object ? { [K in keyof T & string]: K | `${K}.${Path_0616<T[K], [...D, unknown]>}` }[keyof T & string] : never;
...
type Val_0616<T> = {
  [K in keyof T]: T[K] extends string ? { t: 's'; min: number; max: number }
    ...
    : T[K] extends unknown[] ? { t: 'a'; items: Val_0616<T[K][number]> }
    : T[K] extends object ? { t: 'o'; props: Val_0616<T[K]> }
    : { t: 'u' };
};
...
type CK_0616 = `p06.t16.${'on' | 'off' | 'auto'}.${'dev' | 'stg' | 'prd'}.${'v1' | 'v2' | 'v3'}`;

export type { Entity_06_16, Registry_06_16, CK_0616, EP_0616, EV_0616, DeepMerge_0616 };
