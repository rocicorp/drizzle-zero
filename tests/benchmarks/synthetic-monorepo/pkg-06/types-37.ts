...
type DeepMerge_0637<T, U> = {
  [K in keyof T | keyof U]: K extends keyof T & keyof U
    ? T[K] extends object ? U[K] extends object ? DeepMerge_0637<T[K], U[K]> : U[K] : U[K]
...
type Path_0637<T, D extends unknown[] = []> = D['length'] extends 6 ? never
  : T extends object ? { [K in keyof T & string]: K | `${K}.${Path_0637<T[K], [...D, unknown]>}` }[keyof T & string] : never;
...
type Val_0637<T> = {
  [K in keyof T]: T[K] extends string ? { t: 's'; min: number; max: number }
    ...
    : T[K] extends unknown[] ? { t: 'a'; items: Val_0637<T[K][number]> }
    : T[K] extends object ? { t: 'o'; props: Val_0637<T[K]> }
    : { t: 'u' };
};
...
type CK_0637 = `p06.t37.${'on' | 'off' | 'auto'}.${'dev' | 'stg' | 'prd'}.${'v1' | 'v2' | 'v3'}`;

export type { Entity_06_37, Registry_06_37, CK_0637, EP_0637, EV_0637, DeepMerge_0637 };
