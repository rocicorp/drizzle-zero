...
type DeepMerge_0648<T, U> = {
  [K in keyof T | keyof U]: K extends keyof T & keyof U
    ? T[K] extends object ? U[K] extends object ? DeepMerge_0648<T[K], U[K]> : U[K] : U[K]
...
type Path_0648<T, D extends unknown[] = []> = D['length'] extends 6 ? never
  : T extends object ? { [K in keyof T & string]: K | `${K}.${Path_0648<T[K], [...D, unknown]>}` }[keyof T & string] : never;
...
type Val_0648<T> = {
  [K in keyof T]: T[K] extends string ? { t: 's'; min: number; max: number }
    ...
    : T[K] extends unknown[] ? { t: 'a'; items: Val_0648<T[K][number]> }
    : T[K] extends object ? { t: 'o'; props: Val_0648<T[K]> }
    : { t: 'u' };
};
...
type CK_0648 = `p06.t48.${'on' | 'off' | 'auto'}.${'dev' | 'stg' | 'prd'}.${'v1' | 'v2' | 'v3'}`;

export type { Entity_06_48, Registry_06_48, CK_0648, EP_0648, EV_0648, DeepMerge_0648 };
