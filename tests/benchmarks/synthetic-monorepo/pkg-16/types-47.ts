...
type DeepMerge_1647<T, U> = {
  [K in keyof T | keyof U]: K extends keyof T & keyof U
    ? T[K] extends object ? U[K] extends object ? DeepMerge_1647<T[K], U[K]> : U[K] : U[K]
...
type Path_1647<T, D extends unknown[] = []> = D['length'] extends 6 ? never
  : T extends object ? { [K in keyof T & string]: K | `${K}.${Path_1647<T[K], [...D, unknown]>}` }[keyof T & string] : never;
...
type Val_1647<T> = {
  [K in keyof T]: T[K] extends string ? { t: 's'; min: number; max: number }
    ...
    : T[K] extends unknown[] ? { t: 'a'; items: Val_1647<T[K][number]> }
    : T[K] extends object ? { t: 'o'; props: Val_1647<T[K]> }
    : { t: 'u' };
};
...
type CK_1647 = `p16.t47.${'on' | 'off' | 'auto'}.${'dev' | 'stg' | 'prd'}.${'v1' | 'v2' | 'v3'}`;

export type { Entity_16_47, Registry_16_47, CK_1647, EP_1647, EV_1647, DeepMerge_1647 };
