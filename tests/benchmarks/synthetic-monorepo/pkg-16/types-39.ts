...
type Path_1639<T, D extends unknown[] = []> = D['length'] extends 6
  ? never
  : T extends object
    ? {
        [K in keyof T & string]: K | `${K}.${Path_1639<T[K], [...D, unknown]>}`;
      }[keyof T & string]
    : never;
  ...
  [K in keyof T]: T[K] extends string
    ...
    : T[K] extends number
      ...
      : T[K] extends boolean
        ? {t: 'b'; def: boolean}
        : T[K] extends unknown[]
          ? {t: 'a'; items: Val_1639<T[K][number]>}
          : T[K] extends object
            ? {t: 'o'; props: Val_1639<T[K]>}
            : {t: 'u'};
};
...
type CK_1639 =
  `p16.t39.${'on' | 'off' | 'auto'}.${'dev' | 'stg' | 'prd'}.${'v1' | 'v2' | 'v3'}`;
