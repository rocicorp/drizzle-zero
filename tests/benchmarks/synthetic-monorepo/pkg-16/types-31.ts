...
type DeepMerge_1631<T, U> = {
  [K in keyof T | keyof U]: K extends keyof T & keyof U
    ? T[K] extends object ? U[K] extends object ? DeepMerge_1631<T[K], U[K]> : U[K] : U[K]
...
type Path_1631<T, D extends unknown[] = []> = D['length'] extends 6 ? never
  : T extends object ? { [K in keyof T & string]: K | `${K}.${Path_1631<T[K], [...D, unknown]>}` }[keyof T & string] : never;
...
type Val_1631<T> = {
  [K in keyof T]: T[K] extends string ? { t: 's'; min: number; max: number }
    ...
    : T[K] extends unknown[] ? { t: 'a'; items: Val_1631<T[K][number]> }
    : T[K] extends object ? { t: 'o'; props: Val_1631<T[K]> }
    : { t: 'u' };
};
...
type CK_1631 = `p16.t31.${'on' | 'off' | 'auto'}.${'dev' | 'stg' | 'prd'}.${'v1' | 'v2' | 'v3'}`;

export type { Entity_16_31, Registry_16_31, CK_1631, EP_1631, EV_1631, DeepMerge_1631 };
