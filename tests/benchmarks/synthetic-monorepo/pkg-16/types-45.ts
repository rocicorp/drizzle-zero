...
type DeepMerge_1645<T, U> = {
  [K in keyof T | keyof U]: K extends keyof T & keyof U
    ? T[K] extends object ? U[K] extends object ? DeepMerge_1645<T[K], U[K]> : U[K] : U[K]
...
type Path_1645<T, D extends unknown[] = []> = D['length'] extends 6 ? never
  : T extends object ? { [K in keyof T & string]: K | `${K}.${Path_1645<T[K], [...D, unknown]>}` }[keyof T & string] : never;
...
type Val_1645<T> = {
  [K in keyof T]: T[K] extends string ? { t: 's'; min: number; max: number }
    ...
    : T[K] extends unknown[] ? { t: 'a'; items: Val_1645<T[K][number]> }
    : T[K] extends object ? { t: 'o'; props: Val_1645<T[K]> }
    : { t: 'u' };
};
...
type CK_1645 = `p16.t45.${'on' | 'off' | 'auto'}.${'dev' | 'stg' | 'prd'}.${'v1' | 'v2' | 'v3'}`;

export type { Entity_16_45, Registry_16_45, CK_1645, EP_1645, EV_1645, DeepMerge_1645 };
