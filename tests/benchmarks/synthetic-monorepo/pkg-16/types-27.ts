...
type DeepMerge_1627<T, U> = {
  [K in keyof T | keyof U]: K extends keyof T & keyof U
    ? T[K] extends object ? U[K] extends object ? DeepMerge_1627<T[K], U[K]> : U[K] : U[K]
...
type Path_1627<T, D extends unknown[] = []> = D['length'] extends 6 ? never
  : T extends object ? { [K in keyof T & string]: K | `${K}.${Path_1627<T[K], [...D, unknown]>}` }[keyof T & string] : never;
...
type Val_1627<T> = {
  [K in keyof T]: T[K] extends string ? { t: 's'; min: number; max: number }
    ...
    : T[K] extends unknown[] ? { t: 'a'; items: Val_1627<T[K][number]> }
    : T[K] extends object ? { t: 'o'; props: Val_1627<T[K]> }
    : { t: 'u' };
};
...
type CK_1627 = `p16.t27.${'on' | 'off' | 'auto'}.${'dev' | 'stg' | 'prd'}.${'v1' | 'v2' | 'v3'}`;

export type { Entity_16_27, Registry_16_27, CK_1627, EP_1627, EV_1627, DeepMerge_1627 };
