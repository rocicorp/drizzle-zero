...
type DeepMerge_1630<T, U> = {
  [K in keyof T | keyof U]: K extends keyof T & keyof U
    ? T[K] extends object ? U[K] extends object ? DeepMerge_1630<T[K], U[K]> : U[K] : U[K]
...
type Path_1630<T, D extends unknown[] = []> = D['length'] extends 6 ? never
  : T extends object ? { [K in keyof T & string]: K | `${K}.${Path_1630<T[K], [...D, unknown]>}` }[keyof T & string] : never;
...
type Val_1630<T> = {
  [K in keyof T]: T[K] extends string ? { t: 's'; min: number; max: number }
    ...
    : T[K] extends unknown[] ? { t: 'a'; items: Val_1630<T[K][number]> }
    : T[K] extends object ? { t: 'o'; props: Val_1630<T[K]> }
    : { t: 'u' };
};
...
type CK_1630 = `p16.t30.${'on' | 'off' | 'auto'}.${'dev' | 'stg' | 'prd'}.${'v1' | 'v2' | 'v3'}`;

export type { Entity_16_30, Registry_16_30, CK_1630, EP_1630, EV_1630, DeepMerge_1630 };
