...
type DeepMerge_1628<T, U> = {
  [K in keyof T | keyof U]: K extends keyof T & keyof U
    ? T[K] extends object ? U[K] extends object ? DeepMerge_1628<T[K], U[K]> : U[K] : U[K]
...
type Path_1628<T, D extends unknown[] = []> = D['length'] extends 6 ? never
  : T extends object ? { [K in keyof T & string]: K | `${K}.${Path_1628<T[K], [...D, unknown]>}` }[keyof T & string] : never;
...
type Val_1628<T> = {
  [K in keyof T]: T[K] extends string ? { t: 's'; min: number; max: number }
    ...
    : T[K] extends unknown[] ? { t: 'a'; items: Val_1628<T[K][number]> }
    : T[K] extends object ? { t: 'o'; props: Val_1628<T[K]> }
    : { t: 'u' };
};
...
type CK_1628 = `p16.t28.${'on' | 'off' | 'auto'}.${'dev' | 'stg' | 'prd'}.${'v1' | 'v2' | 'v3'}`;

export type { Entity_16_28, Registry_16_28, CK_1628, EP_1628, EV_1628, DeepMerge_1628 };
