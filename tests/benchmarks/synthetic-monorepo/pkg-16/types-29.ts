...
type DeepMerge_1629<T, U> = {
  [K in keyof T | keyof U]: K extends keyof T & keyof U
    ? T[K] extends object ? U[K] extends object ? DeepMerge_1629<T[K], U[K]> : U[K] : U[K]
...
type Path_1629<T, D extends unknown[] = []> = D['length'] extends 6 ? never
  : T extends object ? { [K in keyof T & string]: K | `${K}.${Path_1629<T[K], [...D, unknown]>}` }[keyof T & string] : never;
...
type Val_1629<T> = {
  [K in keyof T]: T[K] extends string ? { t: 's'; min: number; max: number }
    ...
    : T[K] extends unknown[] ? { t: 'a'; items: Val_1629<T[K][number]> }
    : T[K] extends object ? { t: 'o'; props: Val_1629<T[K]> }
    : { t: 'u' };
};
...
type CK_1629 = `p16.t29.${'on' | 'off' | 'auto'}.${'dev' | 'stg' | 'prd'}.${'v1' | 'v2' | 'v3'}`;

export type { Entity_16_29, Registry_16_29, CK_1629, EP_1629, EV_1629, DeepMerge_1629 };
