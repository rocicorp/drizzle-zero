...
type DeepMerge_2332<T, U> = {
  [K in keyof T | keyof U]: K extends keyof T & keyof U
    ? T[K] extends object ? U[K] extends object ? DeepMerge_2332<T[K], U[K]> : U[K] : U[K]
...
type Path_2332<T, D extends unknown[] = []> = D['length'] extends 6 ? never
  : T extends object ? { [K in keyof T & string]: K | `${K}.${Path_2332<T[K], [...D, unknown]>}` }[keyof T & string] : never;
...
type Val_2332<T> = {
  [K in keyof T]: T[K] extends string ? { t: 's'; min: number; max: number }
    ...
    : T[K] extends unknown[] ? { t: 'a'; items: Val_2332<T[K][number]> }
    : T[K] extends object ? { t: 'o'; props: Val_2332<T[K]> }
    : { t: 'u' };
};
...
type CK_2332 = `p23.t32.${'on' | 'off' | 'auto'}.${'dev' | 'stg' | 'prd'}.${'v1' | 'v2' | 'v3'}`;

export type { Entity_23_32, Registry_23_32, CK_2332, EP_2332, EV_2332, DeepMerge_2332 };
