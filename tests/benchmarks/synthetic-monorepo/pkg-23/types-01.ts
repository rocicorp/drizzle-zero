...
type DeepMerge_2301<T, U> = {
  [K in keyof T | keyof U]: K extends keyof T & keyof U
    ? T[K] extends object ? U[K] extends object ? DeepMerge_2301<T[K], U[K]> : U[K] : U[K]
...
type Path_2301<T, D extends unknown[] = []> = D['length'] extends 6 ? never
  : T extends object ? { [K in keyof T & string]: K | `${K}.${Path_2301<T[K], [...D, unknown]>}` }[keyof T & string] : never;
...
type Val_2301<T> = {
  [K in keyof T]: T[K] extends string ? { t: 's'; min: number; max: number }
    ...
    : T[K] extends unknown[] ? { t: 'a'; items: Val_2301<T[K][number]> }
    : T[K] extends object ? { t: 'o'; props: Val_2301<T[K]> }
    : { t: 'u' };
};
...
type CK_2301 = `p23.t01.${'on' | 'off' | 'auto'}.${'dev' | 'stg' | 'prd'}.${'v1' | 'v2' | 'v3'}`;

export type { Entity_23_01, Registry_23_01, CK_2301, EP_2301, EV_2301, DeepMerge_2301 };
