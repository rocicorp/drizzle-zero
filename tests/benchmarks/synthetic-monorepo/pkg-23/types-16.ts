...
type DeepMerge_2316<T, U> = {
  [K in keyof T | keyof U]: K extends keyof T & keyof U
    ? T[K] extends object ? U[K] extends object ? DeepMerge_2316<T[K], U[K]> : U[K] : U[K]
...
type Path_2316<T, D extends unknown[] = []> = D['length'] extends 6 ? never
  : T extends object ? { [K in keyof T & string]: K | `${K}.${Path_2316<T[K], [...D, unknown]>}` }[keyof T & string] : never;
...
type Val_2316<T> = {
  [K in keyof T]: T[K] extends string ? { t: 's'; min: number; max: number }
    ...
    : T[K] extends unknown[] ? { t: 'a'; items: Val_2316<T[K][number]> }
    : T[K] extends object ? { t: 'o'; props: Val_2316<T[K]> }
    : { t: 'u' };
};
...
type CK_2316 = `p23.t16.${'on' | 'off' | 'auto'}.${'dev' | 'stg' | 'prd'}.${'v1' | 'v2' | 'v3'}`;

export type { Entity_23_16, Registry_23_16, CK_2316, EP_2316, EV_2316, DeepMerge_2316 };
