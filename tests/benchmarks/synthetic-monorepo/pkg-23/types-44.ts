...
type DeepMerge_2344<T, U> = {
  [K in keyof T | keyof U]: K extends keyof T & keyof U
    ? T[K] extends object ? U[K] extends object ? DeepMerge_2344<T[K], U[K]> : U[K] : U[K]
...
type Path_2344<T, D extends unknown[] = []> = D['length'] extends 6 ? never
  : T extends object ? { [K in keyof T & string]: K | `${K}.${Path_2344<T[K], [...D, unknown]>}` }[keyof T & string] : never;
...
type Val_2344<T> = {
  [K in keyof T]: T[K] extends string ? { t: 's'; min: number; max: number }
    ...
    : T[K] extends unknown[] ? { t: 'a'; items: Val_2344<T[K][number]> }
    : T[K] extends object ? { t: 'o'; props: Val_2344<T[K]> }
    : { t: 'u' };
};
...
type CK_2344 = `p23.t44.${'on' | 'off' | 'auto'}.${'dev' | 'stg' | 'prd'}.${'v1' | 'v2' | 'v3'}`;

export type { Entity_23_44, Registry_23_44, CK_2344, EP_2344, EV_2344, DeepMerge_2344 };
