...
type DeepMerge_2324<T, U> = {
  [K in keyof T | keyof U]: K extends keyof T & keyof U
    ? T[K] extends object ? U[K] extends object ? DeepMerge_2324<T[K], U[K]> : U[K] : U[K]
...
type Path_2324<T, D extends unknown[] = []> = D['length'] extends 6 ? never
  : T extends object ? { [K in keyof T & string]: K | `${K}.${Path_2324<T[K], [...D, unknown]>}` }[keyof T & string] : never;
...
type Val_2324<T> = {
  [K in keyof T]: T[K] extends string ? { t: 's'; min: number; max: number }
    ...
    : T[K] extends unknown[] ? { t: 'a'; items: Val_2324<T[K][number]> }
    : T[K] extends object ? { t: 'o'; props: Val_2324<T[K]> }
    : { t: 'u' };
};
...
type CK_2324 = `p23.t24.${'on' | 'off' | 'auto'}.${'dev' | 'stg' | 'prd'}.${'v1' | 'v2' | 'v3'}`;

export type { Entity_23_24, Registry_23_24, CK_2324, EP_2324, EV_2324, DeepMerge_2324 };
