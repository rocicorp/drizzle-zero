...
type DeepMerge_3207<T, U> = {
  [K in keyof T | keyof U]: K extends keyof T & keyof U
    ? T[K] extends object ? U[K] extends object ? DeepMerge_3207<T[K], U[K]> : U[K] : U[K]
...
type Path_3207<T, D extends unknown[] = []> = D['length'] extends 6 ? never
  : T extends object ? { [K in keyof T & string]: K | `${K}.${Path_3207<T[K], [...D, unknown]>}` }[keyof T & string] : never;
...
type Val_3207<T> = {
  [K in keyof T]: T[K] extends string ? { t: 's'; min: number; max: number }
    ...
    : T[K] extends unknown[] ? { t: 'a'; items: Val_3207<T[K][number]> }
    : T[K] extends object ? { t: 'o'; props: Val_3207<T[K]> }
    : { t: 'u' };
};
...
type CK_3207 = `p32.t07.${'on' | 'off' | 'auto'}.${'dev' | 'stg' | 'prd'}.${'v1' | 'v2' | 'v3'}`;

export type { Entity_32_07, Registry_32_07, CK_3207, EP_3207, EV_3207, DeepMerge_3207 };
