...
type DeepMerge_3242<T, U> = {
  [K in keyof T | keyof U]: K extends keyof T & keyof U
    ? T[K] extends object ? U[K] extends object ? DeepMerge_3242<T[K], U[K]> : U[K] : U[K]
...
type Path_3242<T, D extends unknown[] = []> = D['length'] extends 6 ? never
  : T extends object ? { [K in keyof T & string]: K | `${K}.${Path_3242<T[K], [...D, unknown]>}` }[keyof T & string] : never;
...
type Val_3242<T> = {
  [K in keyof T]: T[K] extends string ? { t: 's'; min: number; max: number }
    ...
    : T[K] extends unknown[] ? { t: 'a'; items: Val_3242<T[K][number]> }
    : T[K] extends object ? { t: 'o'; props: Val_3242<T[K]> }
    : { t: 'u' };
};
...
type CK_3242 = `p32.t42.${'on' | 'off' | 'auto'}.${'dev' | 'stg' | 'prd'}.${'v1' | 'v2' | 'v3'}`;

export type { Entity_32_42, Registry_32_42, CK_3242, EP_3242, EV_3242, DeepMerge_3242 };
