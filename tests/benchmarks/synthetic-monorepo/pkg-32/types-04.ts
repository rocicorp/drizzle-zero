...
type DeepMerge_3204<T, U> = {
  [K in keyof T | keyof U]: K extends keyof T & keyof U
    ? T[K] extends object ? U[K] extends object ? DeepMerge_3204<T[K], U[K]> : U[K] : U[K]
...
type Path_3204<T, D extends unknown[] = []> = D['length'] extends 6 ? never
  : T extends object ? { [K in keyof T & string]: K | `${K}.${Path_3204<T[K], [...D, unknown]>}` }[keyof T & string] : never;
...
type Val_3204<T> = {
  [K in keyof T]: T[K] extends string ? { t: 's'; min: number; max: number }
    ...
    : T[K] extends unknown[] ? { t: 'a'; items: Val_3204<T[K][number]> }
    : T[K] extends object ? { t: 'o'; props: Val_3204<T[K]> }
    : { t: 'u' };
};
...
type CK_3204 = `p32.t04.${'on' | 'off' | 'auto'}.${'dev' | 'stg' | 'prd'}.${'v1' | 'v2' | 'v3'}`;

export type { Entity_32_04, Registry_32_04, CK_3204, EP_3204, EV_3204, DeepMerge_3204 };
