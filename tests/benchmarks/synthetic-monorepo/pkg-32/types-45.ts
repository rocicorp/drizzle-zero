...
type DeepMerge_3245<T, U> = {
  [K in keyof T | keyof U]: K extends keyof T & keyof U
    ? T[K] extends object ? U[K] extends object ? DeepMerge_3245<T[K], U[K]> : U[K] : U[K]
...
type Path_3245<T, D extends unknown[] = []> = D['length'] extends 6 ? never
  : T extends object ? { [K in keyof T & string]: K | `${K}.${Path_3245<T[K], [...D, unknown]>}` }[keyof T & string] : never;
...
type Val_3245<T> = {
  [K in keyof T]: T[K] extends string ? { t: 's'; min: number; max: number }
    ...
    : T[K] extends unknown[] ? { t: 'a'; items: Val_3245<T[K][number]> }
    : T[K] extends object ? { t: 'o'; props: Val_3245<T[K]> }
    : { t: 'u' };
};
...
type CK_3245 = `p32.t45.${'on' | 'off' | 'auto'}.${'dev' | 'stg' | 'prd'}.${'v1' | 'v2' | 'v3'}`;

export type { Entity_32_45, Registry_32_45, CK_3245, EP_3245, EV_3245, DeepMerge_3245 };
