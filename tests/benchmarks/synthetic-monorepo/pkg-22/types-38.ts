...
type DeepMerge_2238<T, U> = {
  [K in keyof T | keyof U]: K extends keyof T & keyof U
    ? T[K] extends object ? U[K] extends object ? DeepMerge_2238<T[K], U[K]> : U[K] : U[K]
...
type Path_2238<T, D extends unknown[] = []> = D['length'] extends 6 ? never
  : T extends object ? { [K in keyof T & string]: K | `${K}.${Path_2238<T[K], [...D, unknown]>}` }[keyof T & string] : never;
...
type Val_2238<T> = {
  [K in keyof T]: T[K] extends string ? { t: 's'; min: number; max: number }
    ...
    : T[K] extends unknown[] ? { t: 'a'; items: Val_2238<T[K][number]> }
    : T[K] extends object ? { t: 'o'; props: Val_2238<T[K]> }
    : { t: 'u' };
};
...
type CK_2238 = `p22.t38.${'on' | 'off' | 'auto'}.${'dev' | 'stg' | 'prd'}.${'v1' | 'v2' | 'v3'}`;

export type { Entity_22_38, Registry_22_38, CK_2238, EP_2238, EV_2238, DeepMerge_2238 };
