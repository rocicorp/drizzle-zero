...
type DeepMerge_2212<T, U> = {
  [K in keyof T | keyof U]: K extends keyof T & keyof U
    ? T[K] extends object ? U[K] extends object ? DeepMerge_2212<T[K], U[K]> : U[K] : U[K]
...
type Path_2212<T, D extends unknown[] = []> = D['length'] extends 6 ? never
  : T extends object ? { [K in keyof T & string]: K | `${K}.${Path_2212<T[K], [...D, unknown]>}` }[keyof T & string] : never;
...
type Val_2212<T> = {
  [K in keyof T]: T[K] extends string ? { t: 's'; min: number; max: number }
    ...
    : T[K] extends unknown[] ? { t: 'a'; items: Val_2212<T[K][number]> }
    : T[K] extends object ? { t: 'o'; props: Val_2212<T[K]> }
    : { t: 'u' };
};
...
type CK_2212 = `p22.t12.${'on' | 'off' | 'auto'}.${'dev' | 'stg' | 'prd'}.${'v1' | 'v2' | 'v3'}`;

export type { Entity_22_12, Registry_22_12, CK_2212, EP_2212, EV_2212, DeepMerge_2212 };
