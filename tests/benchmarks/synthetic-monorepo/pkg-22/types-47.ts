...
type DeepMerge_2247<T, U> = {
  [K in keyof T | keyof U]: K extends keyof T & keyof U
    ? T[K] extends object ? U[K] extends object ? DeepMerge_2247<T[K], U[K]> : U[K] : U[K]
...
type Path_2247<T, D extends unknown[] = []> = D['length'] extends 6 ? never
  : T extends object ? { [K in keyof T & string]: K | `${K}.${Path_2247<T[K], [...D, unknown]>}` }[keyof T & string] : never;
...
type Val_2247<T> = {
  [K in keyof T]: T[K] extends string ? { t: 's'; min: number; max: number }
    ...
    : T[K] extends unknown[] ? { t: 'a'; items: Val_2247<T[K][number]> }
    : T[K] extends object ? { t: 'o'; props: Val_2247<T[K]> }
    : { t: 'u' };
};
...
type CK_2247 = `p22.t47.${'on' | 'off' | 'auto'}.${'dev' | 'stg' | 'prd'}.${'v1' | 'v2' | 'v3'}`;

export type { Entity_22_47, Registry_22_47, CK_2247, EP_2247, EV_2247, DeepMerge_2247 };
