...
type DeepMerge_2224<T, U> = {
  [K in keyof T | keyof U]: K extends keyof T & keyof U
    ? T[K] extends object ? U[K] extends object ? DeepMerge_2224<T[K], U[K]> : U[K] : U[K]
...
type Path_2224<T, D extends unknown[] = []> = D['length'] extends 6 ? never
  : T extends object ? { [K in keyof T & string]: K | `${K}.${Path_2224<T[K], [...D, unknown]>}` }[keyof T & string] : never;
...
type Val_2224<T> = {
  [K in keyof T]: T[K] extends string ? { t: 's'; min: number; max: number }
    ...
    : T[K] extends unknown[] ? { t: 'a'; items: Val_2224<T[K][number]> }
    : T[K] extends object ? { t: 'o'; props: Val_2224<T[K]> }
    : { t: 'u' };
};
...
type CK_2224 = `p22.t24.${'on' | 'off' | 'auto'}.${'dev' | 'stg' | 'prd'}.${'v1' | 'v2' | 'v3'}`;

export type { Entity_22_24, Registry_22_24, CK_2224, EP_2224, EV_2224, DeepMerge_2224 };
