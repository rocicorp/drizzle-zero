...
type DeepMerge_2241<T, U> = {
  [K in keyof T | keyof U]: K extends keyof T & keyof U
    ? T[K] extends object ? U[K] extends object ? DeepMerge_2241<T[K], U[K]> : U[K] : U[K]
...
type Path_2241<T, D extends unknown[] = []> = D['length'] extends 6 ? never
  : T extends object ? { [K in keyof T & string]: K | `${K}.${Path_2241<T[K], [...D, unknown]>}` }[keyof T & string] : never;
...
type Val_2241<T> = {
  [K in keyof T]: T[K] extends string ? { t: 's'; min: number; max: number }
    ...
    : T[K] extends unknown[] ? { t: 'a'; items: Val_2241<T[K][number]> }
    : T[K] extends object ? { t: 'o'; props: Val_2241<T[K]> }
    : { t: 'u' };
};
...
type CK_2241 = `p22.t41.${'on' | 'off' | 'auto'}.${'dev' | 'stg' | 'prd'}.${'v1' | 'v2' | 'v3'}`;

export type { Entity_22_41, Registry_22_41, CK_2241, EP_2241, EV_2241, DeepMerge_2241 };
