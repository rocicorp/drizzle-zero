...
type DeepMerge_2144<T, U> = {
  [K in keyof T | keyof U]: K extends keyof T & keyof U
    ? T[K] extends object ? U[K] extends object ? DeepMerge_2144<T[K], U[K]> : U[K] : U[K]
...
type Path_2144<T, D extends unknown[] = []> = D['length'] extends 6 ? never
  : T extends object ? { [K in keyof T & string]: K | `${K}.${Path_2144<T[K], [...D, unknown]>}` }[keyof T & string] : never;
...
type Val_2144<T> = {
  [K in keyof T]: T[K] extends string ? { t: 's'; min: number; max: number }
    ...
    : T[K] extends unknown[] ? { t: 'a'; items: Val_2144<T[K][number]> }
    : T[K] extends object ? { t: 'o'; props: Val_2144<T[K]> }
    : { t: 'u' };
};
...
type CK_2144 = `p21.t44.${'on' | 'off' | 'auto'}.${'dev' | 'stg' | 'prd'}.${'v1' | 'v2' | 'v3'}`;

export type { Entity_21_44, Registry_21_44, CK_2144, EP_2144, EV_2144, DeepMerge_2144 };
