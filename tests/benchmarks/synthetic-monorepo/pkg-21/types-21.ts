...
type DeepMerge_2121<T, U> = {
  [K in keyof T | keyof U]: K extends keyof T & keyof U
    ? T[K] extends object ? U[K] extends object ? DeepMerge_2121<T[K], U[K]> : U[K] : U[K]
...
type Path_2121<T, D extends unknown[] = []> = D['length'] extends 6 ? never
  : T extends object ? { [K in keyof T & string]: K | `${K}.${Path_2121<T[K], [...D, unknown]>}` }[keyof T & string] : never;
...
type Val_2121<T> = {
  [K in keyof T]: T[K] extends string ? { t: 's'; min: number; max: number }
    ...
    : T[K] extends unknown[] ? { t: 'a'; items: Val_2121<T[K][number]> }
    : T[K] extends object ? { t: 'o'; props: Val_2121<T[K]> }
    : { t: 'u' };
};
...
type CK_2121 = `p21.t21.${'on' | 'off' | 'auto'}.${'dev' | 'stg' | 'prd'}.${'v1' | 'v2' | 'v3'}`;

export type { Entity_21_21, Registry_21_21, CK_2121, EP_2121, EV_2121, DeepMerge_2121 };
