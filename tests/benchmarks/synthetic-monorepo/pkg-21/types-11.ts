...
type DeepMerge_2111<T, U> = {
  [K in keyof T | keyof U]: K extends keyof T & keyof U
    ? T[K] extends object ? U[K] extends object ? DeepMerge_2111<T[K], U[K]> : U[K] : U[K]
...
type Path_2111<T, D extends unknown[] = []> = D['length'] extends 6 ? never
  : T extends object ? { [K in keyof T & string]: K | `${K}.${Path_2111<T[K], [...D, unknown]>}` }[keyof T & string] : never;
...
type Val_2111<T> = {
  [K in keyof T]: T[K] extends string ? { t: 's'; min: number; max: number }
    ...
    : T[K] extends unknown[] ? { t: 'a'; items: Val_2111<T[K][number]> }
    : T[K] extends object ? { t: 'o'; props: Val_2111<T[K]> }
    : { t: 'u' };
};
...
type CK_2111 = `p21.t11.${'on' | 'off' | 'auto'}.${'dev' | 'stg' | 'prd'}.${'v1' | 'v2' | 'v3'}`;

export type { Entity_21_11, Registry_21_11, CK_2111, EP_2111, EV_2111, DeepMerge_2111 };
