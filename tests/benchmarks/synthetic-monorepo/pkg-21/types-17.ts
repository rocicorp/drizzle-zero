...
type DeepMerge_2117<T, U> = {
  [K in keyof T | keyof U]: K extends keyof T & keyof U
    ? T[K] extends object ? U[K] extends object ? DeepMerge_2117<T[K], U[K]> : U[K] : U[K]
...
type Path_2117<T, D extends unknown[] = []> = D['length'] extends 6 ? never
  : T extends object ? { [K in keyof T & string]: K | `${K}.${Path_2117<T[K], [...D, unknown]>}` }[keyof T & string] : never;
...
type Val_2117<T> = {
  [K in keyof T]: T[K] extends string ? { t: 's'; min: number; max: number }
    ...
    : T[K] extends unknown[] ? { t: 'a'; items: Val_2117<T[K][number]> }
    : T[K] extends object ? { t: 'o'; props: Val_2117<T[K]> }
    : { t: 'u' };
};
...
type CK_2117 = `p21.t17.${'on' | 'off' | 'auto'}.${'dev' | 'stg' | 'prd'}.${'v1' | 'v2' | 'v3'}`;

export type { Entity_21_17, Registry_21_17, CK_2117, EP_2117, EV_2117, DeepMerge_2117 };
