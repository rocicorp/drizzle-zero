...
type DeepMerge_2138<T, U> = {
  [K in keyof T | keyof U]: K extends keyof T & keyof U
    ? T[K] extends object ? U[K] extends object ? DeepMerge_2138<T[K], U[K]> : U[K] : U[K]
...
type Path_2138<T, D extends unknown[] = []> = D['length'] extends 6 ? never
  : T extends object ? { [K in keyof T & string]: K | `${K}.${Path_2138<T[K], [...D, unknown]>}` }[keyof T & string] : never;
...
type Val_2138<T> = {
  [K in keyof T]: T[K] extends string ? { t: 's'; min: number; max: number }
    ...
    : T[K] extends unknown[] ? { t: 'a'; items: Val_2138<T[K][number]> }
    : T[K] extends object ? { t: 'o'; props: Val_2138<T[K]> }
    : { t: 'u' };
};
...
type CK_2138 = `p21.t38.${'on' | 'off' | 'auto'}.${'dev' | 'stg' | 'prd'}.${'v1' | 'v2' | 'v3'}`;

export type { Entity_21_38, Registry_21_38, CK_2138, EP_2138, EV_2138, DeepMerge_2138 };
