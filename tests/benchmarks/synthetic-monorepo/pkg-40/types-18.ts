...
type DeepMerge_4018<T, U> = {
  [K in keyof T | keyof U]: K extends keyof T & keyof U
    ? T[K] extends object ? U[K] extends object ? DeepMerge_4018<T[K], U[K]> : U[K] : U[K]
...
type Path_4018<T, D extends unknown[] = []> = D['length'] extends 6 ? never
  : T extends object ? { [K in keyof T & string]: K | `${K}.${Path_4018<T[K], [...D, unknown]>}` }[keyof T & string] : never;
...
type Val_4018<T> = {
  [K in keyof T]: T[K] extends string ? { t: 's'; min: number; max: number }
    ...
    : T[K] extends unknown[] ? { t: 'a'; items: Val_4018<T[K][number]> }
    : T[K] extends object ? { t: 'o'; props: Val_4018<T[K]> }
    : { t: 'u' };
};
...
type CK_4018 = `p40.t18.${'on' | 'off' | 'auto'}.${'dev' | 'stg' | 'prd'}.${'v1' | 'v2' | 'v3'}`;

export type { Entity_40_18, Registry_40_18, CK_4018, EP_4018, EV_4018, DeepMerge_4018 };
