...
type DeepMerge_4043<T, U> = {
  [K in keyof T | keyof U]: K extends keyof T & keyof U
    ? T[K] extends object ? U[K] extends object ? DeepMerge_4043<T[K], U[K]> : U[K] : U[K]
...
type Path_4043<T, D extends unknown[] = []> = D['length'] extends 6 ? never
  : T extends object ? { [K in keyof T & string]: K | `${K}.${Path_4043<T[K], [...D, unknown]>}` }[keyof T & string] : never;
...
type Val_4043<T> = {
  [K in keyof T]: T[K] extends string ? { t: 's'; min: number; max: number }
    ...
    : T[K] extends unknown[] ? { t: 'a'; items: Val_4043<T[K][number]> }
    : T[K] extends object ? { t: 'o'; props: Val_4043<T[K]> }
    : { t: 'u' };
};
...
type CK_4043 = `p40.t43.${'on' | 'off' | 'auto'}.${'dev' | 'stg' | 'prd'}.${'v1' | 'v2' | 'v3'}`;

export type { Entity_40_43, Registry_40_43, CK_4043, EP_4043, EV_4043, DeepMerge_4043 };
