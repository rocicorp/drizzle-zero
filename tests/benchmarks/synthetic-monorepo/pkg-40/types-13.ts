...
type DeepMerge_4013<T, U> = {
  [K in keyof T | keyof U]: K extends keyof T & keyof U
    ? T[K] extends object ? U[K] extends object ? DeepMerge_4013<T[K], U[K]> : U[K] : U[K]
...
type Path_4013<T, D extends unknown[] = []> = D['length'] extends 6 ? never
  : T extends object ? { [K in keyof T & string]: K | `${K}.${Path_4013<T[K], [...D, unknown]>}` }[keyof T & string] : never;
...
type Val_4013<T> = {
  [K in keyof T]: T[K] extends string ? { t: 's'; min: number; max: number }
    ...
    : T[K] extends unknown[] ? { t: 'a'; items: Val_4013<T[K][number]> }
    : T[K] extends object ? { t: 'o'; props: Val_4013<T[K]> }
    : { t: 'u' };
};
...
type CK_4013 = `p40.t13.${'on' | 'off' | 'auto'}.${'dev' | 'stg' | 'prd'}.${'v1' | 'v2' | 'v3'}`;

export type { Entity_40_13, Registry_40_13, CK_4013, EP_4013, EV_4013, DeepMerge_4013 };
