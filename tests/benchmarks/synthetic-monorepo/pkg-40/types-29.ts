...
type DeepMerge_4029<T, U> = {
  [K in keyof T | keyof U]: K extends keyof T & keyof U
    ? T[K] extends object ? U[K] extends object ? DeepMerge_4029<T[K], U[K]> : U[K] : U[K]
...
type Path_4029<T, D extends unknown[] = []> = D['length'] extends 6 ? never
  : T extends object ? { [K in keyof T & string]: K | `${K}.${Path_4029<T[K], [...D, unknown]>}` }[keyof T & string] : never;
...
type Val_4029<T> = {
  [K in keyof T]: T[K] extends string ? { t: 's'; min: number; max: number }
    ...
    : T[K] extends unknown[] ? { t: 'a'; items: Val_4029<T[K][number]> }
    : T[K] extends object ? { t: 'o'; props: Val_4029<T[K]> }
    : { t: 'u' };
};
...
type CK_4029 = `p40.t29.${'on' | 'off' | 'auto'}.${'dev' | 'stg' | 'prd'}.${'v1' | 'v2' | 'v3'}`;

export type { Entity_40_29, Registry_40_29, CK_4029, EP_4029, EV_4029, DeepMerge_4029 };
