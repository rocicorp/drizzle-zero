...
type DeepMerge_4003<T, U> = {
  [K in keyof T | keyof U]: K extends keyof T & keyof U
    ? T[K] extends object ? U[K] extends object ? DeepMerge_4003<T[K], U[K]> : U[K] : U[K]
...
type Path_4003<T, D extends unknown[] = []> = D['length'] extends 6 ? never
  : T extends object ? { [K in keyof T & string]: K | `${K}.${Path_4003<T[K], [...D, unknown]>}` }[keyof T & string] : never;
...
type Val_4003<T> = {
  [K in keyof T]: T[K] extends string ? { t: 's'; min: number; max: number }
    ...
    : T[K] extends unknown[] ? { t: 'a'; items: Val_4003<T[K][number]> }
    : T[K] extends object ? { t: 'o'; props: Val_4003<T[K]> }
    : { t: 'u' };
};
...
type CK_4003 = `p40.t03.${'on' | 'off' | 'auto'}.${'dev' | 'stg' | 'prd'}.${'v1' | 'v2' | 'v3'}`;

export type { Entity_40_03, Registry_40_03, CK_4003, EP_4003, EV_4003, DeepMerge_4003 };
