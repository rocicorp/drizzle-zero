...
type DeepMerge_3311<T, U> = {
  [K in keyof T | keyof U]: K extends keyof T & keyof U
    ? T[K] extends object ? U[K] extends object ? DeepMerge_3311<T[K], U[K]> : U[K] : U[K]
...
type Path_3311<T, D extends unknown[] = []> = D['length'] extends 6 ? never
  : T extends object ? { [K in keyof T & string]: K | `${K}.${Path_3311<T[K], [...D, unknown]>}` }[keyof T & string] : never;
...
type Val_3311<T> = {
  [K in keyof T]: T[K] extends string ? { t: 's'; min: number; max: number }
    ...
    : T[K] extends unknown[] ? { t: 'a'; items: Val_3311<T[K][number]> }
    : T[K] extends object ? { t: 'o'; props: Val_3311<T[K]> }
    : { t: 'u' };
};
...
type CK_3311 = `p33.t11.${'on' | 'off' | 'auto'}.${'dev' | 'stg' | 'prd'}.${'v1' | 'v2' | 'v3'}`;

export type { Entity_33_11, Registry_33_11, CK_3311, EP_3311, EV_3311, DeepMerge_3311 };
