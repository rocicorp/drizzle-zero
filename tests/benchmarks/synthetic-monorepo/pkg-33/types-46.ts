...
type DeepMerge_3346<T, U> = {
  [K in keyof T | keyof U]: K extends keyof T & keyof U
    ? T[K] extends object ? U[K] extends object ? DeepMerge_3346<T[K], U[K]> : U[K] : U[K]
...
type Path_3346<T, D extends unknown[] = []> = D['length'] extends 6 ? never
  : T extends object ? { [K in keyof T & string]: K | `${K}.${Path_3346<T[K], [...D, unknown]>}` }[keyof T & string] : never;
...
type Val_3346<T> = {
  [K in keyof T]: T[K] extends string ? { t: 's'; min: number; max: number }
    ...
    : T[K] extends unknown[] ? { t: 'a'; items: Val_3346<T[K][number]> }
    : T[K] extends object ? { t: 'o'; props: Val_3346<T[K]> }
    : { t: 'u' };
};
...
type CK_3346 = `p33.t46.${'on' | 'off' | 'auto'}.${'dev' | 'stg' | 'prd'}.${'v1' | 'v2' | 'v3'}`;

export type { Entity_33_46, Registry_33_46, CK_3346, EP_3346, EV_3346, DeepMerge_3346 };
