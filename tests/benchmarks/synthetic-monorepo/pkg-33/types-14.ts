...
type DeepMerge_3314<T, U> = {
  [K in keyof T | keyof U]: K extends keyof T & keyof U
    ? T[K] extends object ? U[K] extends object ? DeepMerge_3314<T[K], U[K]> : U[K] : U[K]
...
type Path_3314<T, D extends unknown[] = []> = D['length'] extends 6 ? never
  : T extends object ? { [K in keyof T & string]: K | `${K}.${Path_3314<T[K], [...D, unknown]>}` }[keyof T & string] : never;
...
type Val_3314<T> = {
  [K in keyof T]: T[K] extends string ? { t: 's'; min: number; max: number }
    ...
    : T[K] extends unknown[] ? { t: 'a'; items: Val_3314<T[K][number]> }
    : T[K] extends object ? { t: 'o'; props: Val_3314<T[K]> }
    : { t: 'u' };
};
...
type CK_3314 = `p33.t14.${'on' | 'off' | 'auto'}.${'dev' | 'stg' | 'prd'}.${'v1' | 'v2' | 'v3'}`;

export type { Entity_33_14, Registry_33_14, CK_3314, EP_3314, EV_3314, DeepMerge_3314 };
