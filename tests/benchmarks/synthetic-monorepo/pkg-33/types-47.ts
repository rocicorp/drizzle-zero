...
type DeepMerge_3347<T, U> = {
  [K in keyof T | keyof U]: K extends keyof T & keyof U
    ? T[K] extends object ? U[K] extends object ? DeepMerge_3347<T[K], U[K]> : U[K] : U[K]
...
type Path_3347<T, D extends unknown[] = []> = D['length'] extends 6 ? never
  : T extends object ? { [K in keyof T & string]: K | `${K}.${Path_3347<T[K], [...D, unknown]>}` }[keyof T & string] : never;
...
type Val_3347<T> = {
  [K in keyof T]: T[K] extends string ? { t: 's'; min: number; max: number }
    ...
    : T[K] extends unknown[] ? { t: 'a'; items: Val_3347<T[K][number]> }
    : T[K] extends object ? { t: 'o'; props: Val_3347<T[K]> }
    : { t: 'u' };
};
...
type CK_3347 = `p33.t47.${'on' | 'off' | 'auto'}.${'dev' | 'stg' | 'prd'}.${'v1' | 'v2' | 'v3'}`;

export type { Entity_33_47, Registry_33_47, CK_3347, EP_3347, EV_3347, DeepMerge_3347 };
