...
type DeepMerge_3803<T, U> = {
  [K in keyof T | keyof U]: K extends keyof T & keyof U
    ? T[K] extends object ? U[K] extends object ? DeepMerge_3803<T[K], U[K]> : U[K] : U[K]
...
type Path_3803<T, D extends unknown[] = []> = D['length'] extends 6 ? never
  : T extends object ? { [K in keyof T & string]: K | `${K}.${Path_3803<T[K], [...D, unknown]>}` }[keyof T & string] : never;
...
type Val_3803<T> = {
  [K in keyof T]: T[K] extends string ? { t: 's'; min: number; max: number }
    ...
    : T[K] extends unknown[] ? { t: 'a'; items: Val_3803<T[K][number]> }
    : T[K] extends object ? { t: 'o'; props: Val_3803<T[K]> }
    : { t: 'u' };
};
...
type CK_3803 = `p38.t03.${'on' | 'off' | 'auto'}.${'dev' | 'stg' | 'prd'}.${'v1' | 'v2' | 'v3'}`;

export type { Entity_38_03, Registry_38_03, CK_3803, EP_3803, EV_3803, DeepMerge_3803 };
