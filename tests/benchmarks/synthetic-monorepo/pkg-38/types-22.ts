...
type DeepMerge_3822<T, U> = {
  [K in keyof T | keyof U]: K extends keyof T & keyof U
    ? T[K] extends object ? U[K] extends object ? DeepMerge_3822<T[K], U[K]> : U[K] : U[K]
...
type Path_3822<T, D extends unknown[] = []> = D['length'] extends 6 ? never
  : T extends object ? { [K in keyof T & string]: K | `${K}.${Path_3822<T[K], [...D, unknown]>}` }[keyof T & string] : never;
...
type Val_3822<T> = {
  [K in keyof T]: T[K] extends string ? { t: 's'; min: number; max: number }
    ...
    : T[K] extends unknown[] ? { t: 'a'; items: Val_3822<T[K][number]> }
    : T[K] extends object ? { t: 'o'; props: Val_3822<T[K]> }
    : { t: 'u' };
};
...
type CK_3822 = `p38.t22.${'on' | 'off' | 'auto'}.${'dev' | 'stg' | 'prd'}.${'v1' | 'v2' | 'v3'}`;

export type { Entity_38_22, Registry_38_22, CK_3822, EP_3822, EV_3822, DeepMerge_3822 };
