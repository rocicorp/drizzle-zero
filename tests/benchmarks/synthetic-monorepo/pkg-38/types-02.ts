...
type DeepMerge_3802<T, U> = {
  [K in keyof T | keyof U]: K extends keyof T & keyof U
    ? T[K] extends object ? U[K] extends object ? DeepMerge_3802<T[K], U[K]> : U[K] : U[K]
...
type Path_3802<T, D extends unknown[] = []> = D['length'] extends 6 ? never
  : T extends object ? { [K in keyof T & string]: K | `${K}.${Path_3802<T[K], [...D, unknown]>}` }[keyof T & string] : never;
...
type Val_3802<T> = {
  [K in keyof T]: T[K] extends string ? { t: 's'; min: number; max: number }
    ...
    : T[K] extends unknown[] ? { t: 'a'; items: Val_3802<T[K][number]> }
    : T[K] extends object ? { t: 'o'; props: Val_3802<T[K]> }
    : { t: 'u' };
};
...
type CK_3802 = `p38.t02.${'on' | 'off' | 'auto'}.${'dev' | 'stg' | 'prd'}.${'v1' | 'v2' | 'v3'}`;

export type { Entity_38_02, Registry_38_02, CK_3802, EP_3802, EV_3802, DeepMerge_3802 };
