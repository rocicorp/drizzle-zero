...
type DeepMerge_3828<T, U> = {
  [K in keyof T | keyof U]: K extends keyof T & keyof U
    ? T[K] extends object ? U[K] extends object ? DeepMerge_3828<T[K], U[K]> : U[K] : U[K]
...
type Path_3828<T, D extends unknown[] = []> = D['length'] extends 6 ? never
  : T extends object ? { [K in keyof T & string]: K | `${K}.${Path_3828<T[K], [...D, unknown]>}` }[keyof T & string] : never;
...
type Val_3828<T> = {
  [K in keyof T]: T[K] extends string ? { t: 's'; min: number; max: number }
    ...
    : T[K] extends unknown[] ? { t: 'a'; items: Val_3828<T[K][number]> }
    : T[K] extends object ? { t: 'o'; props: Val_3828<T[K]> }
    : { t: 'u' };
};
...
type CK_3828 = `p38.t28.${'on' | 'off' | 'auto'}.${'dev' | 'stg' | 'prd'}.${'v1' | 'v2' | 'v3'}`;

export type { Entity_38_28, Registry_38_28, CK_3828, EP_3828, EV_3828, DeepMerge_3828 };
