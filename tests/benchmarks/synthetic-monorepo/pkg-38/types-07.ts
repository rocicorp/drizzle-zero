...
type DeepMerge_3807<T, U> = {
  [K in keyof T | keyof U]: K extends keyof T & keyof U
    ? T[K] extends object ? U[K] extends object ? DeepMerge_3807<T[K], U[K]> : U[K] : U[K]
...
type Path_3807<T, D extends unknown[] = []> = D['length'] extends 6 ? never
  : T extends object ? { [K in keyof T & string]: K | `${K}.${Path_3807<T[K], [...D, unknown]>}` }[keyof T & string] : never;
...
type Val_3807<T> = {
  [K in keyof T]: T[K] extends string ? { t: 's'; min: number; max: number }
    ...
    : T[K] extends unknown[] ? { t: 'a'; items: Val_3807<T[K][number]> }
    : T[K] extends object ? { t: 'o'; props: Val_3807<T[K]> }
    : { t: 'u' };
};
...
type CK_3807 = `p38.t07.${'on' | 'off' | 'auto'}.${'dev' | 'stg' | 'prd'}.${'v1' | 'v2' | 'v3'}`;

export type { Entity_38_07, Registry_38_07, CK_3807, EP_3807, EV_3807, DeepMerge_3807 };
